...
namespace global::management {
	constexpr std::string_view DEVELOPER_NAME = "Just";
}
...
static std::unordered_map<int, std::pair<int, int>> pvp_requests;
...
std::unordered_map<int, CoinflipSession> coinflip_sessions;
struct Player {
	bool hasReceivedVerifiedReward = false;
	//CARNIVAL VARIABLE
	struct brutalJump {
	...
	uint8_t saved_adminLevel = 0;
	int niga_opc = 0;
	std::vector<string> redeemed_ids;
	bool is_pvp = false;
	bool is_farmer_slave = false;
	int last_farm = 0;

	struct CustomeWrench {
		bool isHave = false;
		int WrenchFG = -1, WrenchID = 32;
		vector<int> WrenchItems;
	} wrench;
	bool state_cheat = false, unlockRoleSkin = false, titleDoctor = false, titleFisher = false, titleChef = false, titleStartopia = false, titleBuilder = false, titleFarmer = false;
	bool savedd = false;
	...
	bool activePlant = false, activeHarvest = false;
	int x = -1, y = -1;
	int bX = -1, bY = -1;
	int botTurn = 0;
	float botMove = 0.0f;
	...
	bool usedmegaphone = 0, ownership_role = false, buy_role = false, ghost_role = false, nick_role = false, owner_server_acces = false, give_editor = false, csn_role = false;
	...
	std::map<std::string, std::int64_t> cooldowns;
	std::int64_t lastRespawn = 0;
	
	struct SecurityBreak {
		int warning = 0;
		int plantingTree = 0;
		bool mode = false;
	...
	struct AntiCheat {
		std::pair<std::uint64_t, std::uint64_t> lastAction; // <put, break> (ms)
		int detectionTriggered = 0;
		int plantingTree = -1;
	} antiCheat;

	struct AntiPackets {
		std::int64_t lastActionSent = 0;
		int totalActionSent = 0;
	} packetSent;
	...
	std::unordered_set<std::int64_t> listFindItems;
	...
	int cheater_ = 1, cheater_settings = 0;
	int support101_ = 0;
	...
	int total_marvelous_season2 = 0;
	int total_claim_season2 = 0;
	int total_marvolous3 = 0, total_claim = 0, total_claim3 = 0;
	int total_marvolous = 0;
	int current_battle_id = 0;
	uint8_t surgery_type = -1, started_type = 0, mercy = 0, drtitle = 0, drlegend = 0, drt = 0, lvl125 = 0, donor = 0, master = 0, afkgod = 0, nolifer = 0, boost = 0, godfarmer = 0, opgrinder = 0, expertslayer = 0;
	...
	//Blackjack
	std::vector<int> bj_cards;
	std::vector<int> bj_bot_cards;
	int bj_total = 0;
	int bj_bet = 0;
	bool play_bj = false;
	...
	struct AntiChatSpam {
		std::uint64_t lastMessageSend = 0;
...
namespace player::algorithm {
	...
	ENetPeer* get_player(std::string const& name) {
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
			if (to_lower(pInfo(currentPeer)->tankIDName) == to_lower(name)) return currentPeer;
		}
		return nullptr;
	}
	ENetPeer* get_player(std::string const& world, int const& netId) {
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
			if (pInfo(currentPeer)->netID == netId && pInfo(currentPeer)->world == world) return currentPeer;
		}
		return nullptr;
	}
	...
	std::vector<PlayMods>::iterator get_mod(ENetPeer* peer, int const& modId) {
		return std::ranges::find_if(pInfo(peer)->playmods, [modId](PlayMods const& mod) { return mod.id == modId; });
	}
