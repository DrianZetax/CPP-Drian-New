...
ENetPeer* get_peer_by_netid(int netID) {
	for (ENetPeer* peer = server->peers; peer < &server->peers[server->peerCount]; ++peer) {
		if (peer && peer->state == ENET_PEER_STATE_CONNECTED && peer->data) {
...
void NPCMov(PlayerMoving* data, int netID, float newX, float newY, const std::string& world) {
	data->packetType = 0;
	data->netID = netID;
	data->x = newX;
	data->y = newY;
	...
	data->XSpeed = 0;
	data->YSpeed = 0;
	data->plantingTree = 0x8;

	BYTE* raw = packPlayerMoving(data); // Sudah benar

	// Kirim ke semua peer dalam world
	player::algorithm::loop_world(world, [&](ENetPeer* peer) {
		if (!peer || peer->state != ENET_PEER_STATE_CONNECTED || !peer->data) return;
		send_raw(peer, 4, raw, 56, ENET_PACKET_FLAG_RELIABLE);
		});

	delete[] raw;
}
void NPCBreakBlock(int netID, int tileX, int tileY, const std::string& world) {
	PlayerMoving data;
	data.packetType = 3; // Type untuk break block
	data.netID = netID;
	data.x = tileX * 32; // Koordinat world dalam pixel
	data.y = tileY * 32;
	data.punchX = tileX;
	data.punchY = tileY;
	data.XSpeed = 0;
	data.YSpeed = 0;
	data.plantingTree = 18; // 18 = break block
...
int retRandomValues(int rangeMin, int rangeMax) {
	// Ensure that rangeMin is less than rangeMax
	if (rangeMin > rangeMax) std::swap(rangeMin, rangeMax);

	std::uniform_int_distribution<std::mt19937::result_type> udist(rangeMin, rangeMax);
	std::mt19937 rng;
	std::mt19937::result_type const seedval = chrono::steady_clock::now().time_since_epoch().count();
	rng.seed(seedval);

	// Get the random value
	int randomValue = udist(rng);

	return randomValue;
}
long long int last_carni_time, last_carnig_time, last_brutal_time, last_spiky_time = 0;
void loop_worlds() {
	if (f_saving_ == false) {
		...
		const double frameDelta = 0.0125; // 50ms = 20 FPS
		static double lastUpdateTime = 0;

		// Menggunakan chrono untuk deltaTime, mirip dengan ghostThread2
		static auto lastTime = std::chrono::high_resolution_clock::now();
		auto nowSteady = std::chrono::high_resolution_clock::now();
		std::chrono::duration<double> elapsed = nowSteady - lastTime;
		lastTime = nowSteady;
		double deltaTime = elapsed.count(); // Waktu yang berlalu

		double now = std::chrono::duration_cast<std::chrono::milliseconds>(
			nowSteady.time_since_epoch()).count() / 1000.0;

		// Menggunakan frameDelta sebagai interval update pergerakan/target
		if (now - lastUpdateTime >= frameDelta) {
			lastUpdateTime = now;

			for (World& world : worlds) {
				// Hapus NPC dengan health <= 0
				world.npcx.erase(std::remove_if(world.npcx.begin(), world.npcx.end(), [](const WorldNPCx& n) {
					return n.m_health <= 0;
					}), world.npcx.end());

				for (auto& npc : world.npcx) {
					if (!npc.enabled) continue;

					if (npc.id == TYPE_GHOST) {
						npc.m_speed = 80.0f; // Kecepatan default hantu
					}
					else {
						npc.m_speed = 50.0f;
					}

					// Logika asli: Perbarui target berdasarkan m_time_move (interval waktu)
					// Kita pertahankan, tapi kita ubah logika perhitungannya di dalam.
					if (std::chrono::duration_cast<std::chrono::milliseconds>(nowSteady - npc.m_last_move_time).count() >= npc.m_time_move) {

						// --- START LOGIKA PERGERAKAN GHOST DARI KODE LAMA ---
						if (npc.id == TYPE_GHOST) {
							// 1. Simpan posisi saat ini sebagai posisi terakhir
							npc.m_last_position = npc.m_position;

							// 2. Hitung target baru secara acak, mirip dengan logika lama
							float newX = retRandomValues(-160, 160); // Jarak acak x
							float newY = retRandomValues(-160, 160); // Jarak acak y

							npc.m_target_position.m_x = npc.m_last_position.m_x + newX;
							npc.m_target_position.m_y = npc.m_last_position.m_y + newY;

							// 3. Atur kecepatan acak dan hitung durasi
							npc.m_speed = retRandomValues(33.0f, 44.0f); // Kecepatan acak

							npc.m_distance = std::hypot(npc.m_target_position.m_x - npc.m_last_position.m_x,
								npc.m_target_position.m_y - npc.m_last_position.m_y);

							// Jika jaraknya sangat kecil, hindari pembagian dengan nol
							npc.m_duration = npc.m_distance / max(npc.m_speed, 1.0f);

							// Penting: Reset timer pergerakan
							npc.m_last_move_time = nowSteady;

							// Atur m_time_move (interval waktu tunggu pergerakan berikutnya) ke m_duration
							// agar pergerakan berikutnya terjadi setelah pergerakan saat ini selesai.
							// Kita tambahkan jeda kecil (misal 50ms) agar ada waktu 'istirahat'
							npc.m_time_move = (uint32_t)(npc.m_duration * 1000.0f) + 50;
						}
						// --- END LOGIKA PERGERAKAN GHOST DARI KODE LAMA ---
						else {
							// Logika asli untuk NPC non-hantu (target pemain terdekat)
							npc.m_last_move_time = nowSteady;
							npc.m_last_position = npc.m_position;

							ENetPeer* targetPeer = nullptr;
							float closestDistance = FLT_MAX;
							// ... (Logika pencarian pemain terdekat) ...
							for (ENetPeer* peer = server->peers; peer < &server->peers[server->peerCount]; ++peer) {
								if (!peer || peer->state != ENET_PEER_STATE_CONNECTED || !peer->data) continue;
								if (pInfo(peer)->world != world.name) continue;

								float dx = pInfo(peer)->x - npc.m_position.m_x;
								float dy = pInfo(peer)->y - npc.m_position.m_y;
							...
							if (targetPeer) {
								npc.m_target_position.m_x = pInfo(targetPeer)->x;
								npc.m_target_position.m_y = pInfo(targetPeer)->y;
							}
							else {
								// Target acak jika tidak ada pemain
								npc.m_target_position.m_x = (float)(rand() % (32 * 99));
								npc.m_target_position.m_y = (float)(rand() % (32 * 59));
							}

							npc.m_distance = std::hypot(npc.m_target_position.m_x - npc.m_position.m_x,
								npc.m_target_position.m_y - npc.m_position.m_y);
							npc.m_duration = npc.m_distance / max(npc.m_speed, 1.0f);

							// Atur interval waktu tunggu pergerakan berikutnya (misal 1 detik)
							npc.m_time_move = 1000;
						}
					}

					// Hitung waktu yang telah berlalu
					// Jika waktu pergerakan saat ini sudah selesai, elapsed akan dihitung dari last_move_time yang baru.
					float elapsed_sec = std::chrono::duration<float>(nowSteady - npc.m_last_move_time).count();

					// Interpolasi pergerakan
					float t = clamp(elapsed_sec / max(npc.m_duration, 0.01f), 0.0f, 1.0f);
					float curr_x = npc.m_last_position.m_x + (npc.m_target_position.m_x - npc.m_last_position.m_x) * t;
					float curr_y = npc.m_last_position.m_y + (npc.m_target_position.m_y - npc.m_last_position.m_y) * t;

					npc.m_position.m_x = curr_x;
					npc.m_position.m_y = curr_y;

					// Broadcast movement ke semua player di world
					for (ENetPeer* peer = server->peers; peer < &server->peers[server->peerCount]; ++peer) {
						if (!peer || peer->state != ENET_PEER_STATE_CONNECTED || !peer->data) continue;
						if (pInfo(peer)->world != world.name) continue;

						PlayerMoving data{};
						// Asumsi NPCMov akan memanggil GUP untuk update posisi
						NPCMov(&data, npc.uid, curr_x, curr_y, world.name);
					}
				}
			}
		}
		...
		//			// Menghitung jarak untuk interpolasi
		//			float deltaX = npc.m_position.m_x - npc.m_last_position.m_x;
		//			float deltaY = npc.m_position.m_y - npc.m_last_position.m_y;
		//			float distance = std::sqrt(deltaX * deltaX + deltaY * deltaY);
		//			float durationSec = distance / max(npc.m_speed, 1.0f);
		//			float elapsed = duration<float>(nowSteady - npc.m_last_move_time).count();

		//			if (elapsed >= durationSec || durationSec == 0.0f) {
		//				npc.m_last_position = npc.m_position;

		//				// Logika mengejar target
		//				if (npc.m_chase_target != -1) {
		//					ENetPeer* targetPeer = get_peer_by_netid(npc.m_chase_target);
		//					if (targetPeer && pInfo(targetPeer)->world == world.name) {
		//						npc.m_position = { static_cast<float>(pInfo(targetPeer)->x), static_cast<float>(pInfo(targetPeer)->y) };
		//						npc.m_last_move_time = nowSteady;
		//					}
		//					else {
		//						npc.m_chase_target = -1; // Target tidak ditemukan
		//					}
		//				}
		//				else {
		//					// Gerakan random
		//					float to_x = npc.m_position.m_x + static_cast<float>(random(-256, 256));
		//					float to_y = npc.m_position.m_y + static_cast<float>(random(-256, 256));
		//					to_x = std::clamp(to_x, 16.0f, 99 * 32.0f + 16);
		//					to_y = std::clamp(to_y, 16.0f, 59 * 32.0f + 16);

		//					npc.m_position = { to_x, to_y };
		//					npc.m_last_move_time = nowSteady;
		//				}
		//			}

		//			// Interpolasi pergerakan NPC untuk smooth
		//			float interp = clamp(elapsed / max(durationSec, 0.01f), 0.0f, 1.0f);
		//			float curr_x = npc.m_last_position.m_x + (npc.m_position.m_x - npc.m_last_position.m_x) * interp;
		//			float curr_y = npc.m_last_position.m_y + (npc.m_position.m_y - npc.m_last_position.m_y) * interp;

		//			int tile_x = static_cast<int>(curr_x / 32);
		//			int tile_y = static_cast<int>(curr_y / 32);

		//			// Kirim pergerakan NPC ke semua player di world
		//			for (ENetPeer* peer = server->peers; peer < &server->peers[server->peerCount]; ++peer) {
		//				if (!peer || peer->state != ENET_PEER_STATE_CONNECTED || !peer->data) continue;
		//				if (pInfo(peer)->world != world.name) continue;

		//				vec2 playerPos = { static_cast<float>(pInfo(peer)->x), static_cast<float>(pInfo(peer)->y) };
		//				float distToPlayer = std::hypot(playerPos.m_x - curr_x, playerPos.m_y - curr_y);

		//				// Jika dekat dengan player, serang
		//				if (distToPlayer < 32.0f) {
		//					gamepacket_t p;
		//					p.Insert("OnTalkBubble");
		//					p.Insert(pInfo(peer)->netID);
		//					p.Insert("`4NPC: Gotcha!");
		//					p.Insert(0);
		//					p.Insert(1);
		//					p.CreatePacket(peer);
		//				}

		//				// Kirim packet pergerakan NPC ke player
		//				PlayerMoving data{};
		//				NPCMov(&data, npc.uid, npc.m_position.m_x, npc.m_position.m_y, world.name);
		//			}
		//		}
		//	}
		//}
		...
		if (last_carni_time - ms_time <= 0 && Carnival) {
			last_carni_time = ms_time + 5000;
			try {
				string name_ = "CARNIVAL";
				bool finna = false, brutal = false, spiky = false;
				vector<string> gamesbusy = {};
				for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
					if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL) continue;
					if (pInfo(currentPeer)->world == name_ and pInfo(currentPeer)->carnival_playing != "") gamesbusy.push_back(pInfo(currentPeer)->carnival_playing);
				}
				...
				if (p != worlds.end()) {
					...
					if (world_->name == name_ && Carnival) {
						int ySize = (int)world_->max_y, xSize = (int)world_->max_x;
						int x = 0, y = 0;

						if (find(gamesbusy.begin(), gamesbusy.end(), "Brutal_Bounce") != gamesbusy.end() && last_brutal_time - ms_time <= 2000) {
							World* world_ = &worlds[p - worlds.begin()];
							for (int i_ = 0; i_ < world_->blocks.size(); i_++) {
								WorldBlock* block_ = &world_->blocks.at(x + (y * 100));
								int x = i_ % 100, y = i_ / 100;
								bool disable = false;
								vector<pair<int, int>> DisableXY;
								{
									DisableXY.push_back(std::make_pair(19, 39));
									DisableXY.push_back(std::make_pair(20, 39));
									DisableXY.push_back(std::make_pair(21, 39));
									DisableXY.push_back(std::make_pair(22, 39));
									DisableXY.push_back(std::make_pair(23, 39));
									DisableXY.push_back(std::make_pair(17, 40));
									DisableXY.push_back(std::make_pair(18, 40));
									DisableXY.push_back(std::make_pair(24, 40));
									DisableXY.push_back(std::make_pair(25, 40));
									DisableXY.push_back(std::make_pair(16, 41));
									DisableXY.push_back(std::make_pair(26, 41));
									DisableXY.push_back(std::make_pair(15, 43));
									DisableXY.push_back(std::make_pair(27, 43));
									DisableXY.push_back(std::make_pair(15, 45));
									DisableXY.push_back(std::make_pair(27, 45));
									DisableXY.push_back(std::make_pair(16, 47));
									DisableXY.push_back(std::make_pair(17, 47));
									DisableXY.push_back(std::make_pair(18, 47));
									DisableXY.push_back(std::make_pair(19, 47));
									DisableXY.push_back(std::make_pair(20, 47));
									DisableXY.push_back(std::make_pair(21, 47));
									DisableXY.push_back(std::make_pair(22, 47));
									DisableXY.push_back(std::make_pair(23, 47));
									DisableXY.push_back(std::make_pair(24, 47));
									DisableXY.push_back(std::make_pair(25, 47));
									DisableXY.push_back(std::make_pair(26, 47));
								}
								for (size_t i = 0; i < DisableXY.size(); ++i) {
									int x_ = DisableXY[i].first;
									int y_ = DisableXY[i].second;
									if (x_ == x && y_ == y) disable = true;
								}
								if (!disable) {
									if (rand() % 2 + 1 == 1 and world_->blocks[(int)x + ((int)y * 100)].fg == 2568) {
										block_->fg = 0;
										block_->bg = 1878;
										PlayerMoving data_{};
						...
						if (find(gamesbusy.begin(), gamesbusy.end(), "Brutal_Bounce") != gamesbusy.end()) brutal = true;
						if (brutal) last_brutal_time = ms_time + 1000;

						if (find(gamesbusy.begin(), gamesbusy.end(), "Spiky_Survival") != gamesbusy.end() && last_spiky_time - ms_time <= 5000) {
							World* world_ = &worlds[p - worlds.begin()];
							for (int i_ = 0; i_ < world_->blocks.size(); i_++) {
								int x = i_ % 100, y = i_ / 100;
								WorldBlock* block_ = &world_->blocks.at(x + (y * 100));
								if (rand() % 2 + 1 == 1 and world_->blocks[i_].fg == 7180) {
									world_->blocks[i_].fg = 7182;
									PlayerMoving data_{};
									data_.packetType = 5, data_.punchX = x, data_.punchY = y, data_.characterState = 0x8;
									BYTE* raw = packPlayerMoving(&data_, 112 + alloc_(world_, block_)); BYTE* blc = raw + 56;
									for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
								...
								if (rand() % 2 + 1 == 1 and world_->blocks[i_].fg == 7182) {
									world_->blocks[i_].fg = 7180;
									PlayerMoving data_{};
									data_.packetType = 5, data_.punchX = x, data_.punchY = y, data_.characterState = 0x8;
									BYTE* raw = packPlayerMoving(&data_, 112 + alloc_(world_, block_)); BYTE* blc = raw + 56;
									for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
						...
						if (find(gamesbusy.begin(), gamesbusy.end(), "Spiky_Survival") != gamesbusy.end()) spiky = true;
						if (spiky) last_spiky_time = ms_time + 1000;

						if (find(gamesbusy.begin(), gamesbusy.end(), "Growganoth_Right") != gamesbusy.end() && last_carnig_time - ms_time <= 5000) {
							World* world_ = &worlds[p - worlds.begin()];
							for (int i_ = 0; i_ < world_->blocks.size(); i_++) {
								int x = i_ % 100, y = i_ / 100;
								WorldBlock* block_ = &world_->blocks.at(x + (y * 100));
						...
						if (find(gamesbusy.begin(), gamesbusy.end(), "Growganoth_Left") != gamesbusy.end() && last_carnig_time - ms_time <= 5000)
						{
							World* world_ = &worlds[p - worlds.begin()];
							for (int i_ = 0; i_ < world_->blocks.size(); i_++) {
								int x = i_ % 100, y = i_ / 100;
						...
						if (finna) last_carnig_time = ms_time + 2000;
						...
						if (find(gamesbusy.begin(), gamesbusy.end(), "Shooter_Left") != gamesbusy.end()) {
							...
							while (slr3 == slreward or slr3 == slr2) slr3 = sllist[rand() % sllist.size()];
						...
						if (find(gamesbusy.begin(), gamesbusy.end(), "Shooter_Right") != gamesbusy.end()) {
							...
							while (rlr3 == rlreward or rlr3 == rlr2) rlr3 = rllist[rand() % rllist.size()];
						...
						if (find(gamesbusy.begin(), gamesbusy.end(), "Growganoth_Left") == gamesbusy.end()) {
							for (int i_ = 0; i_ < world_->blocks.size(); i_++) {
								bool visual = false;
								if (find(gamesbusy.begin(), gamesbusy.end(), "Shooter_Left") != gamesbusy.end()) {
									if (world_->blocks[i_].fg == 1908 and find(sltochange.begin(), sltochange.end(), i_) == sltochange.end() and find(sllist.begin(), sllist.end(), i_) != sllist.end()) {
										if ((world_->blocks[i_].flags & 0x00400000) > 0) world_->blocks[i_].flags = world_->blocks[i_].flags ^ 0x00400000;
										visual = true;
									}
								}
								if (find(gamesbusy.begin(), gamesbusy.end(), "Shooter_Right") != gamesbusy.end()) {
									if (world_->blocks[i_].fg == 1908 and find(rltochange.begin(), rltochange.end(), i_) == rltochange.end() and find(rllist.begin(), rllist.end(), i_) != rllist.end()) {
										if ((world_->blocks[i_].flags & 0x00400000) > 0) world_->blocks[i_].flags = world_->blocks[i_].flags ^ 0x00400000;
										visual = true;
									}
								}
								...
								if (find(gamesbusy.begin(), gamesbusy.end(), "Shooter_Left") != gamesbusy.end()) {
									if (find(sltochange.begin(), sltochange.end(), i_) != sltochange.end()) {
										...
										if (block_->fg == 1908 and (world_->blocks[(int)x + ((int)y * 100)].flags & 0x00400000) == 0) {
											block_->flags = block_->flags | 0x00400000;
											//cout << "4, added flag, " << i_ << endl;
											visual = true;
										}
									}
								}
		...
		if (last_time - ms_time <= 0) {
			...
			for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
				...
				//carnival stuff
				if (pInfo(currentPeer)->carnival_playing == "Growganoth_Left" || pInfo(currentPeer)->carnival_playing == "Growganoth_Right") {
					if ((duration_cast<seconds>(system_clock::now().time_since_epoch())).count() - pInfo(currentPeer)->carnival_entered >= 60) {
						send_bubble(currentPeer, pInfo(currentPeer)->netID, "`8You didn’t make it in time!`` No prize for you!");
						variants::OnConsoleMessage(currentPeer, "`8You didn’t make it in time!`` No prize for you!");
						gamepacket_t p, j;
						carnivalEnd(currentPeer, "end");
						int time = (duration_cast<seconds>(system_clock::now().time_since_epoch())).count() - pInfo(currentPeer)->carnival_entered;
						p.Insert("OnRaceEnd"), p.CreatePacket(currentPeer);
						j.Insert("OnCooldownEnd"), j.CreatePacket(currentPeer);;
					}
				}
				if (pInfo(currentPeer)->carnival_playing == "Card_Left" || pInfo(currentPeer)->carnival_playing == "Card_Right") {
					if ((duration_cast<seconds>(system_clock::now().time_since_epoch())).count() - pInfo(currentPeer)->carnival_entered >= 60) {
						int carniscore = pInfo(currentPeer)->cardGame.TotalCardSuccess;
						int rands = rand() % 6;
						if (carniscore < 8) {
							send_bubble(currentPeer, pInfo(currentPeer)->netID, "`8You only scored " + to_string(carniscore) + " points!`` No prize for you!");
							variants::OnConsoleMessage(currentPeer, "`8You only scored " + to_string(carniscore) + " points!`` No prize for you!");
							variants::OnCountdownStart(currentPeer, pInfo(currentPeer)->netID, 0, 0);
						}
						else {
							send_bubble(currentPeer, pInfo(currentPeer)->netID, "`8You scored " + to_string(carniscore) + " points!`` You win a `2" + items[1796].name + "``!");
							variants::OnConsoleMessage(currentPeer, "`8You scored " + to_string(carniscore) + " points!`` You win a `2" + items[1796].name + "``!");
							variants::OnCountdownStart(currentPeer, pInfo(currentPeer)->netID, 0, 0);
							carnivalPrize(currentPeer);
						}

						for (ENetPeer* currentr = server->peers; currentr < &server->peers[server->peerCount]; currentr++) {
							if (currentr->state != ENET_PEER_STATE_CONNECTED || pInfo(currentr)->carniwaitgame != "Card_Right" || pInfo(currentr)->carniwaitgame != "Card_Left" || pInfo(currentr)->world != pInfo(currentPeer)->world) continue;
							if (pInfo(currentr)->carniwait != 0 && pInfo(currentr)->carniwaitgame == "Card_Left" && pInfo(currentPeer)->carnival_playing == "Card_Left") pInfo(currentr)->carniwait--;
							if (pInfo(currentr)->carniwait == 0 && pInfo(currentr)->carniwaitgame == "Card_Left" && pInfo(currentPeer)->carnival_playing == "Card_Left")	carnivalStart(currentr, "card_left_start");
							if (pInfo(currentr)->carniwait != 0 && pInfo(currentr)->carniwaitgame == "Card_Right" && pInfo(currentPeer)->carnival_playing == "Card_Right") pInfo(currentr)->carniwait--;
							if (pInfo(currentr)->carniwait == 0 && pInfo(currentr)->carniwaitgame == "Card_Right" && pInfo(currentPeer)->carnival_playing == "Card_Right")	carnivalStart(currentr, "card_right_start");
						}
						carnivalEnd(currentPeer, "end");
					}
				}
				if (pInfo(currentPeer)->carnival_playing == "Brutal_Bounce") {
					if ((duration_cast<seconds>(system_clock::now().time_since_epoch())).count() - pInfo(currentPeer)->carnival_entered >= 60) {
						{
							carnivalPrize(currentPeer);
							send_bubble(currentPeer, pInfo(currentPeer)->netID, "`8You managed to survive for 60 seconds!`` You win a `2" + items[1796].name + "``!");
							variants::OnConsoleMessage(currentPeer, "`8You managed to survive for 60 seconds!`` You win a `2" + items[1796].name + "``!");
							variants::OnCountdownStart(currentPeer, pInfo(currentPeer)->netID, 0, 0);
						}

						for (ENetPeer* currentr = server->peers; currentr < &server->peers[server->peerCount]; currentr++) {
							if (currentr->state != ENET_PEER_STATE_CONNECTED || pInfo(currentr)->carniwaitgame != "Brutal_Bounce" || pInfo(currentr)->world != pInfo(currentPeer)->world) continue;
							if (pInfo(currentr)->carniwait != 0 && pInfo(currentr)->carniwaitgame == "Brutal_Bounce" && pInfo(currentPeer)->carnival_playing == "Brutal_Bounce") pInfo(currentr)->carniwait--;
							if (pInfo(currentr)->carniwait == 0 && pInfo(currentr)->carniwaitgame == "Brutal_Bounce" && pInfo(currentPeer)->carnival_playing == "Brutal_Bounce") carnivalStart(currentr, "brutal_bounce_start");
						}
						carnivalEnd(currentPeer, "end");
					}
				}
				if (pInfo(currentPeer)->carnival_playing == "Spiky_Survival") {
					if ((duration_cast<seconds>(system_clock::now().time_since_epoch())).count() - pInfo(currentPeer)->carnival_entered >= 60) {
						{
							carnivalPrize(currentPeer);
							send_bubble(currentPeer, pInfo(currentPeer)->netID, "`8You managed to survive for 60 seconds!`` You win a `2" + items[1796].name + "``!");
						...
						for (ENetPeer* currentr = server->peers; currentr < &server->peers[server->peerCount]; currentr++) {
							if (currentr->state != ENET_PEER_STATE_CONNECTED || pInfo(currentr)->carniwaitgame != "Spiky_Survival" || pInfo(currentr)->world != pInfo(currentPeer)->world) continue;
							if (pInfo(currentr)->carniwait != 0 && pInfo(currentr)->carniwaitgame == "Spiky_Survival" && pInfo(currentPeer)->carnival_playing == "Spiky_Survival") pInfo(currentr)->carniwait--;
							if (pInfo(currentr)->carniwait == 0 && pInfo(currentr)->carniwaitgame == "Spiky_Survival" && pInfo(currentPeer)->carnival_playing == "Spiky_Survival") carnivalStart(currentr, "spiky_survival_start");
						}
						carnivalEnd(currentPeer, "end");
					}
				}
				if (pInfo(currentPeer)->carnival_playing == "Mirror_Maze") {
					if ((duration_cast<seconds>(system_clock::now().time_since_epoch())).count() - pInfo(currentPeer)->carnival_entered >= 60) {
						{
							send_bubble(currentPeer, pInfo(currentPeer)->netID, "`8You didn’t make it in time!`` No prize for you!");
							variants::OnConsoleMessage(currentPeer, "`8You didn’t make it in time!`` No prize for you!");
						...
						for (ENetPeer* currentr = server->peers; currentr < &server->peers[server->peerCount]; currentr++) {
							if (currentr->state != ENET_PEER_STATE_CONNECTED || pInfo(currentr)->carniwaitgame != "Mirror_Maze" || pInfo(currentr)->world != pInfo(currentPeer)->world) continue;
							if (pInfo(currentr)->carniwait != 0 && pInfo(currentr)->carniwaitgame == "Mirror_Maze" && pInfo(currentPeer)->carnival_playing == "Mirror_Maze") pInfo(currentr)->carniwait--;
							if (pInfo(currentr)->carniwait == 0 && pInfo(currentr)->carniwaitgame == "Mirror_Maze" && pInfo(currentPeer)->carnival_playing == "Mirror_Maze") carnivalStart(currentr, "mirror_maze_start");
						}
						carnivalEnd(currentPeer, "end");
					}
				}
				if (pInfo(currentPeer)->carnival_playing == "Shooter_Left" || pInfo(currentPeer)->carnival_playing == "Shooter_Right") {
					if ((duration_cast<seconds>(system_clock::now().time_since_epoch())).count() - pInfo(currentPeer)->carnival_entered >= 60) {
						int carniscore = pInfo(currentPeer)->carnivalscore;
						if (carniscore <= 30) {
							send_bubble(currentPeer, pInfo(currentPeer)->netID, "`8You only scored " + to_string(carniscore) + " points!`` No prize for you!");
							variants::OnConsoleMessage(currentPeer, "`8You only scored " + to_string(carniscore) + " points!`` No prize for you!");
							variants::OnCountdownStart(currentPeer, pInfo(currentPeer)->netID, 0, 0);
						...
						for (ENetPeer* currentr = server->peers; currentr < &server->peers[server->peerCount]; currentr++) {
							if (currentr->state != ENET_PEER_STATE_CONNECTED || pInfo(currentr)->carniwaitgame != "Shooter_Right" || pInfo(currentr)->carniwaitgame != "Shooter_Left" || pInfo(currentr)->world != pInfo(currentPeer)->world) continue;
							if (pInfo(currentr)->carniwait != 0 && pInfo(currentr)->carniwaitgame == "Shooter_Left" && pInfo(currentPeer)->carnival_playing == "Shooter_Left") pInfo(currentr)->carniwait--;
							if (pInfo(currentr)->carniwait == 0 && pInfo(currentr)->carniwaitgame == "Shooter_Left" && pInfo(currentPeer)->carnival_playing == "Shooter_Left")	carnivalStart(currentr, "shooter_left_start");
							if (pInfo(currentr)->carniwait != 0 && pInfo(currentr)->carniwaitgame == "Shooter_Right" && pInfo(currentPeer)->carnival_playing == "Shooter_Right") pInfo(currentr)->carniwait--;
							if (pInfo(currentr)->carniwait == 0 && pInfo(currentr)->carniwaitgame == "Shooter_Right" && pInfo(currentPeer)->carnival_playing == "Shooter_Right") carnivalStart(currentr, "shooter_right_start");
						}
						carnivalEnd(currentPeer, "end");
					}
				}
					...
					if (last_firehouse - ms_time <= 0) {
						...
						if (p != worlds.end()) {
							...
							if (p3 != world->blocks.end()) {
								...
								if (p2 != world->blocks.end()) {
									int x_ = int(p2 - world->blocks.begin()) % 100, y_ = int(p2 - world->blocks.begin()) / 100;
									world->blocks[x_ + (y_ * 100)].flags ^= 0x10000000;
									tile_update(NULL, world, &world->blocks[x_ + (y_ * 100)], x_, y_);
									//apply_tile_visual(world, &world->blocks[x_ + (y_ * 100)], x_, y_, 0x10000000, true);
								}
							}
							if (last_fire_time - ms_time <= 0) {
								if (world->total_fires < 150) {
									...
									if (p2 != world->blocks.end()) {
										...
										if (randomx > 0 && randomx < world->max_x && randomy > 0 && randomy < world->max_y) {
											bool has_fire = world->blocks[randomx + (randomy * 100)].flags & 0x10000000, has_water = world->blocks[randomx + (randomy * 100)].flags & 0x04000000;
											if (world->blocks[randomx + (randomy * 100)].fg != 0 && has_fire == false && has_water == false && items[world->blocks[randomx + (randomy * 100)].fg].blockType != BlockTypes::MAIN_DOOR && items[world->blocks[randomx + (randomy * 100)].fg].blockType != BlockTypes::BEDROCK && world->blocks[randomx + (randomy * 100)].fg != 9570) world->blocks[randomx + (randomy * 100)].flags |= 0x10000000, tile_update(NULL, world, &world->blocks[randomx + (randomy * 100)], randomx, randomy);//apply_tile_visual(world, &world->blocks[randomx + (randomy * 100)], randomx, randomy, 0x10000000);
											else {
												world->blocks[x_ + (y_ * 100)].fire_try++;
												if (world->blocks[x_ + (y_ * 100)].fire_try >= 8) world->blocks[x_ + (y_ * 100)].applied_fire = true, world->blocks[x_ + (y_ * 100)].fire_try = 0;
											}
...
inline void Loopos1() {
	while (true) {
		...
		load_redeem_json();
		Sleep(500);
		roles::name.clear(), roles::displayName.clear(), roles::chat.clear();
		roles::commands.clear(), roles::prefix.clear();

		// Handling roles.xaml
		{
			std::ifstream ifs("settings/roles.xaml");

			if (!std::filesystem::exists("settings/roles.xaml")) {
				ifs.close();
				std::cout << "[!] Missing 'settings/roles.xaml' or it's empty!" << std::endl;
				return;
			}
			else {
				try {
					std::string roleName = "";
					int readLine = 0, toLine = 0;
					bool isWritten = false;
					for (std::string line; getline(ifs, line);) {
						if (line.length() > 3 && line.at(0) != '/' && line.at(1) != '/') {
							auto explodes = explode("|", line);

							if (explodes.at(0) == "<START>") {
								isWritten = true;
								continue;
							}
							else if (explodes.at(0) == "<SPACE>") {
								readLine = false;
								continue;
							}
							else if (explodes.at(0) == "<END>") {
								break;
							}
							if (isWritten) {
								switch (readLine) {
								case 0: {
									roleName = explodes.at(1);
									break;
								}
								case 1: {
									toLine = std::atoi(explodes.at(1).c_str());
									roles::name.insert({ roleName, toLine });
									roles::displayName.insert({ toLine, roleName });
									break;
								}
								case 2: {
									if (explodes.at(1) == "none") {
										roles::prefix.insert({ toLine, "" });
									}
									else roles::prefix.insert({ toLine, explodes.at(1) });
									break;
								}
								case 3: {
									roles::commands.insert({ toLine, explode(",", explodes.at(1)) });
									break;
								}
								case 4: {
									if (explodes.at(1) == "none") {
										roles::chat.insert({ toLine, "" });
									}
									else roles::chat.insert({ toLine, explodes.at(1) });
									break;
								}
								}
								readLine++;
...
inline void Loopos3(ENetPeer* peer) {
	while (true) {
		Sleep(500);
	}
}
namespace packets {
	bool ReceivePacket1(ENetPeer* peer, std::string cch) {
		...
		//ganti break; ke return false;
		 if (packet::contains(cch, "buttonClicked|set_win_chance")) {
			algorithm::DialogReturnParser parser = cch;

			// Ambil data yang di-embed dari dialog 'dice_roll_main'
			int current_balance = calculate_player_wls(peer);
			int win_chance = converttoint(parser.get_embed_data("win_chance"));

			// Kirim dialog Set Chance
			DialogBuilder p;
			p.embed_data("wls_balance", std::to_string(current_balance))
				.embed_data("win_chance", std::to_string(win_chance)) // Kirim win_chance saat ini
				.embed_data("bet_amount", parser.get_value("bet_amount")) // Simpan bet_amount lama
				.add_label_icon(false, 252, "`bDice Roll - Chance")
				.add_smalltext(std::format("`wCurrent Chance: `2{}%", win_chance))
				.add_smalltext("`wSet Win Chance (%):")
				.add_input(2, "new_chance", "", std::to_string(win_chance))
				.add_custom_break()
				.add_button("update_chance", "DONE") // Nama Tombol: update_chance
				.end_dialog("dice_roll_set_chance", "Nevermind", "")
				.send(peer);
			return false;
		}
		 if (packet::contains(cch, "buttonClicked|update_chance")) {
			algorithm::DialogReturnParser parser = cch;

			// Ambil input new_chance (dari input field)
			int new_chance = 0;
			try {
				new_chance = converttoint(parser.get_value("new_chance"));
			}
			catch (...) {
				variants::OnConsoleMessage(peer, "`4Error: Invalid number format for Win Chance.");
				return false;
			}

			// Ambil data lama yang di-embed
			int current_balance = calculate_player_wls(peer);
			std::string old_bet_amount = parser.get_embed_data("bet_amount");
			if (old_bet_amount.empty()) old_bet_amount = "10"; // Default bet 10

			if (new_chance < 1 || new_chance > 98) {
				variants::OnConsoleMessage(peer, "`4Error: Win Chance must be between 1% and 98%.");
				return false;
			}

			double multiplier = (100.0 / (double)new_chance) * 0.96;

			// Kirim kembali dialog dice_roll_main dengan nilai new_chance yang baru
			DialogBuilder p;
			p.embed_data("wls_balance", std::to_string(current_balance))
				.embed_data("win_chance", std::to_string(new_chance)) // Embed nilai BARU
				.add_label_icon(false, 252, "`bDice Roll!")
				.add_smalltext(std::format("`wWin Chance: `2{}%", new_chance))
				.add_smalltext(std::format("`wMultiplier: `2{:.2f}x", multiplier))
				.add_smalltext("`w(Max Win: 10,000 Blue Gem Lock)")
				.add_button("set_win_chance", "Set Win Chance")
				.add_spacer(false)
				.add_smalltext(std::format("`wBet Amount (min. 10, max. 10M)"))
				.add_smalltext(std::format("`wBalance: `2{} World Locks", add_commas(current_balance)))
				.add_input(10, "bet_amount", "Amount:", old_bet_amount) // Pertahankan bet amount lama
				.add_custom_break()
				.add_button("roll_dice", "ROLL!")
				.end_dialog("dice_roll_main", "Nevermind", "")
				.send(peer);

			return false;
			}
		 if (packet::contains(cch, "buttonClicked|roll_dice")) {
			algorithm::DialogReturnParser parser = cch;

			// Ambil input Bet Amount (dari input field)
			int bet_amount = converttoint(parser.get_value("bet_amount"));

			// Ambil WIN_CHANCE dari embed_data
			int win_chance = 50;
			std::string wc_str = parser.get_embed_data("win_chance");
			if (!wc_str.empty()) {
				win_chance = converttoint(wc_str);
			}
			if (win_chance < 1 || win_chance > 98) {
				win_chance = 50;
			}
			...
			const int max_bet = 10000000;
			const int max_payout = 10000000;

			// 1. Validasi
			if (bet_amount < min_bet || bet_amount > max_bet) {
				variants::OnConsoleMessage(peer, "`4Error: Bet amount is outside the min/max range.");
				return false;
			}
			if (current_balance < bet_amount) {
				variants::OnConsoleMessage(peer, "`4Error: Not enough World Locks in your inventory.");
				return false;
			}

			// 2. Kalkulasi
			double multiplier = (100.0 / (double)win_chance) * 0.96;
			double payout = (double)bet_amount * multiplier;
			if (payout > max_payout) {
				payout = max_payout;
			}
			// Cek jika payout kurang dari bet (walaupun jarang terjadi dengan batasan 1-98%)
			if (payout < bet_amount) {
				variants::OnConsoleMessage(peer, "`4Error: Payout is less than bet amount. Adjust win chance.");
				return false;
			}

			// 3. Taruhan (Pengurangan item)
			// *** Panggil deduct_wls_from_inventory(peer, bet_amount); ***

			// 4. Roll
			int roll = rand() % 100 + 1;
			bool player_wins = roll <= win_chance;

			std::string result_message;

			if (player_wins) {
				int win_amount = (int)payout;
				// *** Panggil give_wls_to_inventory(peer, win_amount); ***

				result_message = std::format("`2{}%: YOU WIN! `w(Multiplier: `2{:.2f}x`)", win_chance, multiplier);
				variants::OnConsoleMessage(peer, std::format("`2WIN! You won {} World Locks!", add_commas(win_amount - bet_amount)));
			}
			else {
				result_message = std::format("`4{}%: YOU LOSE! `w(Multiplier: `2{:.2f}x`)", 100 - win_chance, multiplier);
				variants::OnConsoleMessage(peer, std::format("`4LOSS! You lost {} World Locks.", add_commas(bet_amount)));
			}

			// 5. Kirim Ulang Dialog
			int final_balance = calculate_player_wls(peer);
			if (!deduct_wls_from_inventory(peer, bet_amount)) {
				// Jika deduct_wls_from_inventory mengembalikan false (seharusnya dicegah di validasi 1)
				variants::OnConsoleMessage(peer, "`4Critical Error: Failed to deduct world locks!");
				return false;
			}
			...
			if (player_wins) {
				// Menang
				int win_amount = (int)payout;
				int net_win = win_amount - bet_amount;

				// HANYA berikan kemenangan bersih (win_amount)
				if (win_amount > 0) {
					give_wls_to_inventory(peer, win_amount);
				}

				result_message = std::format("`2{}%: YOU WIN! `w(Multiplier: `2{:.2f}x`)", win_chance, multiplier);
				variants::OnConsoleMessage(peer, std::format("`2WIN! You won {} World Locks!", add_commas(net_win)));
			}
			else {
				// Kalah (Taruhan sudah dikurangi di langkah 3)
				result_message = std::format("`4{}%: YOU LOSE! `w(Multiplier: `2{:.2f}x`)", 100 - win_chance, multiplier);
				variants::OnConsoleMessage(peer, std::format("`4LOSS! You lost {} World Locks.", add_commas(bet_amount)));
			}

			DialogBuilder p;
			p.embed_data("wls_balance", std::to_string(final_balance))
				.embed_data("win_chance", std::to_string(win_chance))
				.add_label_icon(false, 252, "`bDice Roll!")
				.add_smalltext(std::format("`wWin Chance: `2{}%", win_chance))
				.add_smalltext(std::format("`wMultiplier: `2{:.2f}x", multiplier))
				.add_smalltext("`w(Max Win: 10,000 Blue Gem Lock)")
				.add_button("set_win_chance", "Set Win Chance")
				.add_spacer(false)
				.add_smalltext(std::format("`wBet Amount (min. 10, max. 10M)"))
				.add_smalltext(std::format("`wBalance: `2{} World Locks", add_commas(final_balance)))
				.add_input(10, "bet_amount", "Amount:", std::to_string(bet_amount))
				.add_custom_break()
				.add_smalltext(result_message)
				.add_button("roll_dice", "ROLL!")
				.end_dialog("dice_roll_main", "Nevermind", "")
				.send(peer);
		...
		if (packet::contains(cch, "dialog_name|black_jack")) {
			algorithm::DialogReturnParser parser = cch;
			std::vector<std::string> lines = explode("|", cch);
			if (lines.size() < 4) return false;
			std::string btn = explode("\n", lines[3])[0];
			int gemamo = converttoint(parser.get_value("gems"));

			if (btn == "play_bj") {

				if (gemamo < 1000 || gemamo > 5000000) {
					send_bubble(peer, pInfo(peer)->netID, "Gems amount too low or too high");
					return false;
				}
				if (pInfo(peer)->gems < gemamo) {
					send_bubble(peer, pInfo(peer)->netID, "Not enough gems.");
					return false;
				}

				// Kurangi gems untuk taruhan
				pInfo(peer)->gems -= gemamo;
				set_bux(peer, pInfo(peer)->gems);

				// Reset data permainan
				pInfo(peer)->bj_cards.clear();
				pInfo(peer)->bj_bot_cards.clear();
				pInfo(peer)->bj_total = 0;
				pInfo(peer)->bj_bet = gemamo;
				pInfo(peer)->play_bj = true;

				// Kartu awal (2 untuk pemain dan bot)
				for (int i = 0; i < 2; i++) {
					int card = rand() % 9 + 1;
					pInfo(peer)->bj_cards.push_back(card);
					int bot_card = rand() % 9 + 1;
					pInfo(peer)->bj_bot_cards.push_back(bot_card);
				}

				// Hitung total pemain
				for (int card : pInfo(peer)->bj_cards)
					pInfo(peer)->bj_total += card;

				DialogBuilder d;
				d.add_label(false, "`bBlack Jack")
					.add_smalltext("`4You bet " + std::to_string(gemamo) + " Gems.")
					.add_smalltext("`wYour Total: `2" + std::to_string(pInfo(peer)->bj_total))
					.add_spacer("small");
				for (int i = 0; i < pInfo(peer)->bj_cards.size(); ++i) {
					d.add_static("card_you_" + std::to_string(i), get_card_icon(pInfo(peer)->bj_cards[i]), "");
					if ((i + 1) % 5 == 0) d.add_custom_break();
				}
				//d.add_spacer("small")
					d.add_custom_break()
					.add_smalltext("`7Bot Cards (hidden)")
					.add_custom_break();
				for (int i = 0; i < 2; ++i) {
					d.add_static("bot_" + std::to_string(i), 7246, "");
					if ((i + 1) % 5 == 0) d.add_custom_break();
				}
				d.add_custom_break()
					.add_button("add_card", "Add Card")
					.add_button("stop_card", "Stop")
					.add_quick_exit()
					.end_dialog("black_jack", "Leave", "")
					.send(peer);
			}

			else if (btn == "add_card") {
				if (!pInfo(peer)->play_bj) return false;
				int card = rand() % 9 + 1;
				pInfo(peer)->bj_cards.push_back(card);
				pInfo(peer)->bj_total = 0;
				for (int card : pInfo(peer)->bj_cards)
				...
				DialogBuilder d;
				d.add_label(false, "`bBlack Jack")
					.add_smalltext("`4You bet " + std::to_string(pInfo(peer)->bj_bet) + " Gems.")
					.add_smalltext("`wYour Total: `2" + std::to_string(pInfo(peer)->bj_total))
					.add_spacer("small");
				for (int i = 0; i < pInfo(peer)->bj_cards.size(); ++i) {
					d.add_static("card_you_" + std::to_string(i), get_card_icon(pInfo(peer)->bj_cards[i]), "");
				...
				d.add_custom_break();
				if (pInfo(peer)->bj_total >= 21) {
					d.add_button("stop_card", "Stop (Auto)")
						.add_smalltext("`6You should stop.");
				}
				else {
					d.add_button("add_card", "Add Card")
						.add_button("stop_card", "Stop Card");
				}
				d.add_quick_exit()
					.end_dialog("black_jack", "Leave", "")
					.send(peer);
			...
			else if (btn == "stop_card") {
				if (!pInfo(peer)->play_bj) return false;
				pInfo(peer)->play_bj = false;

				int bot_total = 0;
				for (int card : pInfo(peer)->bj_bot_cards) bot_total += card;
				while (bot_total < 17) {
					int card = rand() % 9 + 1;
					pInfo(peer)->bj_bot_cards.push_back(card);
					bot_total += card;
				}

				int player = 0;
				for (int card : pInfo(peer)->bj_cards) player += card;
				int bet = pInfo(peer)->bj_bet;
				std::string result;

				if (player > 21 && bot_total > 21)
					result = "`4Both busted! No one wins.";
				else if (player > 21)
					result = "`4You Busted! Bot wins.";
				else if (bot_total > 21) {
					result = "`2Bot Busted! You win!";
					pInfo(peer)->gems += bet * 2;
					set_bux(peer, pInfo(peer)->gems);
				}
				else if (player > bot_total) {
					result = "`2You win!";
					pInfo(peer)->gems += bet * 2;
					set_bux(peer, pInfo(peer)->gems);
				}
				...
				DialogBuilder d;
				d.add_label(false, "`bBlack Jack - Result")
					.add_smalltext("`wYour Total: `2" + std::to_string(player))
					.add_smalltext("`wBot Total: `4" + std::to_string(bot_total))
					.add_spacer("small")
					.add_smalltext("`wYour Cards:");
				for (int i = 0; i < pInfo(peer)->bj_cards.size(); ++i) {
					d.add_static("res_you_" + std::to_string(i), get_card_icon(pInfo(peer)->bj_cards[i]), "");
					if ((i + 1) % 5 == 0) d.add_custom_break();
				}
				d.add_custom_break();
				d.add_custom_break()
					.add_smalltext("`wBot Cards:");
				for (int i = 0; i < pInfo(peer)->bj_bot_cards.size(); ++i) {
					d.add_static("res_bot_" + std::to_string(i), get_card_icon(pInfo(peer)->bj_bot_cards[i]), "");
					if ((i + 1) % 5 == 0) d.add_custom_break();
				}
				d.add_custom_break()
					.add_smalltext(result)
					.add_quick_exit()
					.end_dialog("black_jack_result", "Okay", "")
					.send(peer);
			}
			return false;
		}
else if (cch.find("buttonClicked|redeemgrp_") != std::string::npos) {
	std::string groupName = explode("buttonClicked|redeemgrp_", cch)[1];
	groupName = explode("\n", groupName)[0];

	auto& redeemed = pInfo(peer)->redeemed_ids;

	if (std::find(redeemed.begin(), redeemed.end(), groupName) != redeemed.end()) {
		send_bubble(peer, pInfo(peer)->netID, "`4You already redeemed this bundle!");
		return false;
	}

	// Load redeem.json lagi
	nlohmann::json redeemData;
	std::ifstream file("settings/redeem.json");
	if (!file.is_open()) {
		send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Cannot open redeem.json!");
		return false;
	}
	...
	try {
		...
		send_bubble(peer, pInfo(peer)->netID, "`4Error:`w redeem.json corrupted!");
		file.close();
		return false;
	}

	if (!redeemData.contains(groupName)) {
		send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Redeem bundle not found!");
		return false;
	}

	auto groupArr = redeemData[groupName];
	int totalItems = 0;

	for (auto& arr : groupArr) {
		if (!arr.is_array() || arr.size() < 2) continue;
		int itemid = arr[0].get<int>();
		int value = arr[1].get<int>();
		modify_inventory(peer, itemid, value);
		totalItems++;
	}

	redeemed.push_back(groupName);
	send_bubble(peer, pInfo(peer)->netID,
		"`2Successfully redeemed bundle: `w" + groupName + "`2 containing " + std::to_string(totalItems) + " items!");

	return false;
}
else if (cch.find("buttonClicked|confirm_addredeem") != std::string::npos) {
	algorithm::DialogReturnParser parser = cch;
	std::string groupName = parser.get_value("group_name");
	std::string itemsText = parser.get_value("items");

	if (groupName.empty() || itemsText.empty()) {
		send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Please fill out all fields!");
		return false;
	}

	// Load redeem.json
	nlohmann::json redeemData;
	std::ifstream infile("settings/redeem.json");
	if (infile.is_open()) {
		try { infile >> redeemData; }
		catch (...) {}
		infile.close();
	}
	...
	std::stringstream ss(itemsText);
	std::string pairStr;
	while (std::getline(ss, pairStr, ',')) {
		size_t pos = pairStr.find(':');
		if (pos != std::string::npos) {
			int id = std::stoi(pairStr.substr(0, pos));
			int amt = std::stoi(pairStr.substr(pos + 1));
			if (id > 0 && amt > 0)
				items.push_back({ id, amt });
		}
	}

	if (items.empty()) {
		send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Invalid item format!");
		return false;
	}
	...
	redeemData[groupName] = arr;

	std::ofstream outfile("settings/redeem.json");
	if (outfile.is_open()) {
		outfile << std::setw(4) << redeemData;
		outfile.close();
		send_bubble(peer, pInfo(peer)->netID, "`2Success:`w Added new redeem bundle `" + groupName + "`!");
	}
	else {
		send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Cannot save redeem.json!");
	}
	}

		else if (packet::contains(cch, "dialog_name|create_exchange")) {
			algorithm::DialogReturnParser parser = cch;

			std::string id = parser.get_value("id");
			std::string from_icon_str = parser.get_value("from_icon");
			std::string from_name = parser.get_value("from_name");
			std::string from_amount_str = parser.get_value("from_amount");
			std::string to_icon_str = parser.get_value("to_icon");
			std::string to_name = parser.get_value("to_name");
			std::string to_amount_str = parser.get_value("to_amount");

			// Pastikan semua field diisi
			if (id.empty() || from_icon_str.empty() || from_name.empty() ||
				from_amount_str.empty() || to_icon_str.empty() ||
				to_name.empty() || to_amount_str.empty()) {
				send_bubble(peer, pInfo(peer)->netID, "`4Error:`w All fields must be filled!");
				return false;
			}

			// Validasi agar input angka valid
			int from_icon, from_amount, to_icon, to_amount;
			try {
				from_icon = std::stoi(from_icon_str);
				from_amount = std::stoi(from_amount_str);
				to_icon = std::stoi(to_icon_str);
				to_amount = std::stoi(to_amount_str);
			}
			catch (...) {
				send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Invalid number input!");
				return false;
			}

			if (from_amount <= 0 || to_amount <= 0) {
				send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Amounts must be greater than 0!");
				return false;
			}

			// Baca atau buat file JSON
			nlohmann::json exchanges;
			std::ifstream file("settings/exchange.json");
			if (file.is_open()) {
				try {
					file >> exchanges;
				}
			...
			for (auto& item : exchanges) {
				if (item.value("id", "") == id) {
					send_bubble(peer, pInfo(peer)->netID, "`4Error:`w ID `" + id + "` already exists!");
					return false;
				}
			}
			...
			exchanges.push_back(newItem);

			// Simpan ke file
			std::ofstream outFile("settings/exchange.json");
			outFile << exchanges.dump(4);
			outFile.close();

			send_bubble(peer, pInfo(peer)->netID, "`$Exchange added successfully!");
			return false;
		}
		else if (packet::contains(cch, "buttonClicked|open_exchange")) {
			nlohmann::json exchanges;
			std::ifstream file("settings/exchange.json");
			if (!file.is_open()) {
				send_bubble(peer, pInfo(peer)->netID, "`4Error:`w No exchange data found!");
				return false;
			}
			...
			try {
				...
				send_bubble(peer, pInfo(peer)->netID, "`4Error:`w exchange.json is corrupted!");
				file.close();
				return false;
			}
			file.close();
			...
			if (!exchanges.is_array() || exchanges.empty()) {
				send_bubble(peer, pInfo(peer)->netID, "`oNo exchanges found. Use `/addexchange` first!");
				return false;
			}

			DialogBuilder p;
			p.add_label_icon(false, 2250, "Welcome to Crystal Exchange of " + server_name + " Server")
				.add_smalltext("Here you can exchange your crystals for Wls / Diamond Locks !")
				.add_spacer(false);

			for (auto& item : exchanges) {
				std::string id = item.value("id", "0");
				std::string from_name = item.value("from_name", "Unknown");
				std::string to_name = item.value("to_name", "Unknown");
				int from_icon = item.value("from_icon", 0);
				int to_icon = item.value("to_icon", 0);
				int to_amount = item.value("to_amount", 1);

				// Format blok tampilan seperti contoh /crexchange
				p.add_custom_margin(100, 0)
					.add_static_icon_button("exchange_" + id, from_icon, from_name) // tombol kiri
					.add_custom_margin(100, 0)
					.add_button_icon(482, "", "") // panah tengah
					.add_custom_margin(100, 0)
					.add_static_icon_button("exchange_" + id, to_icon, to_name, std::to_string(to_amount)) // tombol kanan
					.add_custom_margin(100, 0)
					.add_custom_break();
			}

			p.add_quick_exit().send(peer);
			return false;
			}
		else if (cch.find("buttonClicked|exchange_") != std::string::npos) {
			std::string id = explode("buttonClicked|exchange_", cch)[1];
			id = explode("\n", id)[0];

			// Load data exchange.json
			nlohmann::json exchanges;
			std::ifstream file("settings/exchange.json");
			if (!file.is_open()) {
				send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Cannot read exchange data!");
				return false;
			}
			...
			for (auto& item : exchanges) {
				if (item.value("id", "") == id) {
					...
					// Buat dialog baru seperti logika okijhuygtfrd/rcyvtuybino
					DialogBuilder p;
					p.add_label_icon(true, 2250, "Welcome to Crystal Exchange of " + server_name + " Server")
						...
						.add_button("buy_exc_" + id, "Exchange " + from_name)
						.add_quick_exit()
						.send(peer);
			...
			send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Exchange ID not found!");
			return false;
			}
		else if (cch.find("buttonClicked|buy_exc_") != std::string::npos) {
			std::string id = explode("buttonClicked|buy_exc_", cch)[1];
			id = explode("\n", id)[0];

			algorithm::DialogReturnParser parser = cch;
			int countx = converttoint(parser.get_value("count_i"));
			if (countx <= 0) {
				send_bubble(peer, pInfo(peer)->netID, "`4Invalid amount.");
				return false;
			}
			...
			for (auto& item : exchanges) {
				if (item.value("id", "") == id) {
					std::string from_name = item.value("from_name", "Unknown");
					int from_icon = item.value("from_icon", 0);
					int from_amount = item.value("from_amount", 1);
					std::string to_name = item.value("to_name", "Unknown");
					int to_icon = item.value("to_icon", 0);
					int to_amount = item.value("to_amount", 1);

					int total_from = from_amount * countx;
					int total_to = to_amount * countx;

					int owned = inventory_contains(peer, from_icon);
					if (owned < total_from) {
						send_bubble(peer, pInfo(peer)->netID, "`4You don't have enough " + from_name + "!");
						return false;
					}

					int negative_from = -total_from;
					modify_inventory(peer, from_icon, negative_from);
					modify_inventory(peer, to_icon, total_to);

					send_bubble(peer, pInfo(peer)->netID,
						"`2Exchange Success! `w" + std::to_string(total_from) + "x " + from_name +
						" → " + std::to_string(total_to) + "x " + to_name);
					return false;
				}
			}
		...
		else if (cch.find("buttonClicked|opc_buy_") != std::string::npos) {
			std::string id = explode("buttonClicked|opc_buy_", cch)[1];
			id = explode("\n", id)[0];

			// Load data opc_shop.json
			nlohmann::json opc_items;
			std::ifstream file("settings/opc_shop.json");
			if (!file.is_open()) {
				send_bubble(peer, pInfo(peer)->netID, "`4Error:`w Cannot read OPC Shop data!");
				return false;
			}
			...
			try {
				...
				send_bubble(peer, pInfo(peer)->netID, "`4Error:`w opc_shop.json is corrupted!");
				file.close();
				return false;
			}

			for (auto& item : opc_items) {
				if (item.value("id", "") == id) {
					std::string item_name = item.value("item_name", "Unknown Item");
					int item_id = item.value("item_id", 0);
					int price = item.value("price_item", 0);

					// Buat dialog baru untuk konfirmasi pembelian
					DialogBuilder p;
					p.add_label_icon(true, item_id, "Confirm Purchase: " + item_name)
						.add_smalltext("Harga per unit: `w" + std::to_string(price) + " OPC Token`w.")
						.add_custom_break()
						.add_text_input(3, "count_opc", "Amount to Buy (Max 999)", "1")
						.add_button("confirm_opc_" + id, "Buy " + item_name)
						.add_quick_exit()
						.send(peer);
			...
			send_bubble(peer, pInfo(peer)->netID, "`4Error:`w OPC Item ID not found!");
			return false;
			}
		else if (cch.find("buttonClicked|confirm_opc_") != std::string::npos) {
			std::string id = explode("buttonClicked|confirm_opc_", cch)[1];
			id = explode("\n", id)[0];

			algorithm::DialogReturnParser parser = cch;
			int countx = converttoint(parser.get_value("count_opc"));
			if (countx <= 0) {
				send_bubble(peer, pInfo(peer)->netID, "`4Invalid amount.");
				return false;
			}
			...
			for (auto& item : opc_items) {
				if (item.value("id", "") == id) {
					std::string item_name = item.value("item_name", "Unknown Item");
					int item_id = item.value("item_id", 0);
					int price_per_unit = item.value("price_item", 0); // Harga per unit

					int total_price = price_per_unit * countx;

					int owned_opc = pInfo(peer)->niga_opc;

					// Cek apakah OPC Token cukup
					if (owned_opc < total_price) {
						send_bubble(peer, pInfo(peer)->netID, "`4You don't have enough OPC Tokens! `wNeeded: " + std::to_string(total_price));
						return false;
					}

					// Kurangi OPC Token dari akun pemain
					pInfo(peer)->niga_opc -= total_price;

					// Berikan item ke inventori pemain
					modify_inventory(peer, item_id, countx);

					// Kirim notifikasi sukses
					send_bubble(peer, pInfo(peer)->netID,
						"`2OPC Purchase Success! `w" + std::to_string(countx) + "x " + item_name +
						" bought for " + std::to_string(total_price) + " OPC Tokens.");

					// Anda mungkin perlu menambahkan save_player_data(peer) di sini untuk menyimpan saldo OPC yang baru.
			...
			/*if (cch.find("buttonClicked|slot_dialog")) {
				createSlotDialog(peer);
			}*/
			/*if (cch.find("buttonClicked|redeem_menu")) {
				SendCmd(peer, "/redeem");
			}*/
...
ENetPeer* getActivePeer() {
	if (!connectedPeers.empty())
		return connectedPeers.front(); // ambil peer pertama
	return nullptr;
}

// Simulasi event player connect
void onPlayerConnect(ENetPeer* peer) {
	connectedPeers.push_back(peer);
	std::cout << "[+] Peer connected (" << connectedPeers.size() << " total)\n";
}

// Simulasi event player disconnect
void onPlayerDisconnect(ENetPeer* peer) {
	auto it = std::find(connectedPeers.begin(), connectedPeers.end(), peer);
	if (it != connectedPeers.end()) {
		connectedPeers.erase(it);
		std::cout << "[-] Peer disconnected (" << connectedPeers.size() << " remaining)\n";
	}
}
void RunningEvent() {
	...
	while (true) {
		...
		for (auto worldIndex = 0; worldIndex < worlds.size(); worldIndex++) {
			World* world = &worlds[worldIndex];

			if (world->game.started) {
				world->game.timeStarted += deltaTime; //constant do not change
...
int main(int argc, char* argv[]) {
	...
	system("cls"); refresh_wotd_worlds(); { custom::loadItems(); config::one::loadItems(); config::two::loadItems(); config::three::loadItems(); } if (init_enet(server_port) == -1) {}
	else { if (items_dat() == -1) cout << "[!] Item's dat is not found or it's empty? Try to add or report this." << endl; else cout << "ITEMSDAT TOTAL ITEMS : " << setGems(items.size()) << endl; string clists = ""; for (int i = 0; i < clist.WinPS.size(); i++) { clists += clist.WinPS[i] + ", "; } cout << "SERVER RUNNING AT PORT : " << server_port << endl; cout << "THERE IS ALL OF THE CREATOR : " << clists << endl; cout << "SERVER NAME IS : " << server_name << endl; cout << "Loaded blockcoba.json" << endl; cout << "Loaded misc.json" << endl; }
	if (bot_discord == true) {
		InitializeDiscordBot(token_dc);
	}
	startGiveawayScheduler();
	thread a_(RunningEvent);
	a_.detach();
	thread a__(thesaver);
	a__.detach();
	thread b__(Loopos);
	...
	while (true) {
		while (enet_host_service(server, &event, 500) > 0 && (f_saving_ == false and auto_save == false)) {
			...
			case ENET_EVENT_TYPE_CONNECT:
			{
				...
				if (Server_Security.login_count < 100 || Server_Security.update_item_data < 15) {
					for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			case ENET_EVENT_TYPE_RECEIVE:
			{
				...
				case 2:
					if (cch.find("ltoken|") != string::npos) {
...
if (cch.find("action|dialog_return\ndialog_name|set_server_config") != std::string::npos) {
...
else if (cch.find("buttonClicked|yauhsdijokasd") != string::npos) {
	string num1 = explode("\n", explode("num1|", cch)[1])[0];
	string num2 = explode("\n", explode("num2|", cch)[1])[0];
	string operation = explode("\n", explode("operation|", cch)[1])[0];
	if (num1.empty() || num2.empty()) {
		game_packet::send_console_message(peer, "`oError: Please enter both numbers.");
	}
	else {
		...
		if (operation == "+") {
			...
			if (n2 == 0) {
				game_packet::send_console_message(peer, "`oError: Division by zero is not allowed.");
			//	return;
			}
			result = n1 / n2;
		}
		else {
			game_packet::send_console_message(peer, "`oError: Invalid operation selected.");
			//return;
		}

		game_packet::send_console_message(peer, "`oResult: " + std::to_string(result));
	}
}
else if (cch.find("action|dialog_return\ndialog_name|setroles") != string::npos) {
	if (cch.find("buttonClicked|give_") != string::npos) {
		int adminLevel = -1;
		string user = "";
		try {
			adminLevel = stoi(parseText_2(cch, "give_"));
			if (packet::contains(cch, "username|")) user = parseText_2(cch, "username|");
		}
		catch (out_of_range) { break; }
		catch (invalid_argument) { break; }
		catch (exception) { break; }
		user = (user == "" ? pInfo(peer)->last_wrenched : user);
		if (roles::displayName.contains(adminLevel)) {
			string command = "", prefix = roles::prefix[adminLevel], chat = roles::chat[adminLevel], name = roles::displayName[adminLevel];
			for (int i = 0; i < roles::commands[adminLevel].size(); i++) command += roles::commands[adminLevel][i] + (i == roles::commands[adminLevel].size() ? "" : ",");
			DialogBuilder p;
			p.add_label_icon(true, 32, "`4Confirmation");
			p.add_spacer(false);
			p.add_smalltext("`0Role name: `o" + name).add_smalltext("`0Admin level: `o" + to_string(adminLevel)).add_smalltext("`0Prefix:`o " + prefix).add_smalltext("`0Commands: `o" + command).add_smalltext("Chat: " + chat + "-");
			p.add_spacer(false).embed_data("username", user).embed_data("adminlevel", to_string(adminLevel));
			p.add_textbox("Are you sure you want to set " + user + " admin level to " + to_string(adminLevel) + "?");
			p.add_button("confirm", "Yes");
			p.end_dialog("setroles", "Nevermind", "");
			variants::OnDialogRequest(peer, p.to_string(), 250);
			break;
		}
		variants::OnConsoleMessage(peer, "Role do not exists");
		break;
	}
	if (cch.find("buttonClicked|confirm") != string::npos) {

		string usr = "";
		int adminLevel = 0;
		try {
			usr = parseText_2(cch, "username|", "|");
			adminLevel = stoi(parseText_2(cch, "adminlevel|", "|"));
		}
		catch (invalid_argument) { break; }
		catch (out_of_range) { break; }
		if (!usr.empty() && adminLevel >= 0) {
			if (ENetPeer* target = player::algorithm::get_player(usr); target != nullptr) {
				//SendCmd(peer, "/setrole " + usr + " " + to_string(adminLevel), true);
				pInfo(target)->adminLevel = adminLevel;
				update_role(target);
				pInfo(target)->saved_adminLevel = (adminLevel >= 10 ? 0 : adminLevel);
				remove_role_by_adminLevel(target);
				save_player(pInfo(target), false);
				update_clothes(target);
				nick_update_2(peer, NULL);
				variants::OnConsoleMessage(peer, "Success set " + pInfo(target)->tankIDName + " admin level to " + to_string(adminLevel) + " `0[`2ONLINE`0]");
				break;
			}
			else if (filesystem::exists("database/players/" + usr + "_.json")) {
				try {
					ifstream ifs("database/players/" + usr + "_.json", ifstream::binary);
					json j = json::parse(ifs);
					ifs.close();
					j["adminlevel"] = adminLevel;
					j["s_adminlevel"] = (adminLevel >= 10 ? 0 : adminLevel);
					{
						json playMod = j["playmods"].get<json>();
						for (int i = 0; i < playMod.size(); i++) {
							if (playMod[i]["id"].get<uint16_t>() == 125 && adminLevel >= 10) playMod.erase(playMod.begin() + i);
						}
						j["playmods"] = playMod;
					}
					ofstream ofs("database/players/" + usr + "_.json", ofstream::binary);
					ofs << j << endl;
					ofs.close();
					variants::OnConsoleMessage(peer, "Succes set admin level " + to_string(adminLevel) + " to " + j["name"].get<string>() + " `0[`4OFFLINE`0]");
					break;
				}
				catch (exception& error) { variants::OnConsoleMessage(peer, "ERROR FOUND CHECK IT ON YOUR ENET LOGS"); cout << "[INVALID] " << error.what() << endl; break; }
			}
			else { variants::OnConsoleMessage(peer, "There is no player with that name"); break; }
		}
		else {
			variants::OnConsoleMessage(peer, (usr.empty() ? "Please fill the username" : "The admin level can't be less than zero"));
			break;
		}
	}

	}
else if (cch.find("buttonClicked|add_role_info_") != string::npos) {
	std::string username = pInfo(peer)->last_wrenched; // target yang sedang dilihat lewat /info
	if (username.empty()) {
		variants::OnConsoleMessage(peer, "No target selected for role assignment.");
		//return;
	}

	// Cek apakah pemain online
	ENetPeer* target = player::algorithm::get_player(username);
	DialogBuilder p;

	if (target) {
		p.add_label_icon(true, 32, "SET PLAYER ROLE `0(" + player::algorithm::get_display_name(target) + "`0)");
	}
	else {
		if (!filesystem::exists("database/players/" + username + "_.json")) {
			variants::OnConsoleMessage(peer, "That player doesn't exist.");
			//	return;
		}
		...
		json j = json::parse(ifs);
		ifs.close();
		string user = j["name"].get<string>();
		p.add_label_icon(true, 32, "Set Player Role `0(" + user + "`0)");
	}

	pInfo(peer)->last_wrenched = username;

	// Tambahkan semua role yang tersedia
	for (const auto& it : roles::displayName) {
		p.add_button("give_" + to_string(it.first), it.second);
	}

	p.add_quick_exit();
	p.add_spacer(false);
	p.end_dialog("setroles", "Cancel", "");
	variants::OnDialogRequest(peer, p.to_string(), 250);
	//return;
	}
	if (cch.find("dialog_name|customblast_main") != std::string::npos) {
		std::string worldName = "MYBLASTWORLD";
		size_t worldPos = cch.find("world_name|");
		if (worldPos != std::string::npos) {
			size_t start = worldPos + 11;
			size_t end = cch.find('\n', start);
			if (end != std::string::npos) {
				worldName = cch.substr(start, end - start);
			}
			else {
				worldName = cch.substr(start);
			}
			if (!worldName.empty() && worldName.back() == '\r') {
				worldName.pop_back();
			}
		}

		int selectedBlast = 0;
		std::vector<std::pair<std::string, int>> blastButtons = {
			{"blast_9164", 9164}, {"blast_1402", 1402}, {"blast_1060", 1060},
			{"blast_8738", 8738}, {"blast_1532", 1532}, {"blast_830", 830},
			{"blast_10380", 10380}, {"blast_9602", 9602}, {"blast_1136", 1136},
			{"blast_3562", 3562}, {"blast_9582", 9582}, {"blast_7588", 7588},
			{"blast_6666", 6666}, {"blast_942", 942}, {"blast_4774", 4774},
			{"blast_7380", 7380}, {"blast_8556", 8556}
		};

		for (const auto& button : blastButtons) {
			if (cch.find("buttonClicked|" + button.first) != std::string::npos) {
				selectedBlast = button.second;
				break;
			}
		}

		if (cch.find("buttonClicked|edit_templates") != std::string::npos) {
			showBlastTemplateEditor(peer);
			//return;
		}

		if (selectedBlast > 0) {
			DialogBuilder db;
			std::string blastDisplayName = getBlastDisplayName(selectedBlast);
			std::string currentTemplate = getBlastTemplate(selectedBlast);
			std::string defaultTemplate = getDefaultTemplateName(selectedBlast);

			db.add_textbox("`wCreate Blast World: " + blastDisplayName)
				.add_spacer(false)
				.add_textbox("`2World Name: `w" + worldName)
				.add_spacer(false)
				.add_textbox("`5Blast Type: `w" + std::to_string(selectedBlast) + " (" + blastDisplayName + ")")
				.add_spacer(false);

			std::string desc = getBlastDescription(selectedBlast);
			db.add_textbox("`5Description:")
				.add_smalltext(desc)
				.add_spacer(false);

			db.add_textbox("`5Options:")
				.add_text_input(30, "template_name", "Template World", currentTemplate.empty() ? defaultTemplate : currentTemplate)
				.add_smalltext("`7Current: " + (currentTemplate.empty() ? "Default: " + defaultTemplate : currentTemplate))
				.add_spacer(false);

			if (selectedBlast == 9164 || selectedBlast == 1060 || selectedBlast == 8738) {
				db.add_text_input(2, "weather_id", "Weather ID", selectedBlast == 9164 ? "6" : selectedBlast == 1060 ? "6" : "46");
			}

			if (selectedBlast == 6666) {
				db.add_text_input(3, "ore_count", "Ore Count", "40")
					.add_text_input(3, "strong_count", "Strong Blocks", "70");
			}

			db.add_spacer(false)
				.add_button("create_now", "`2Create World Now")
				.add_custom_break()
				.add_button("go_back", "`6Go Back")
				.add_custom_break()
				.add_button("edit_this_template", "`5Edit This Blast's Template")
				.embed_data("blast_id", std::to_string(selectedBlast))
				.embed_data("world_name", worldName)
				.end_dialog("customblast_create", "Cancel", "OK");

			db.send(peer);
		}
	}

	if (cch.find("dialog_name|customblast_create") != std::string::npos) {
		int blastId = 9164;
		std::string worldName = "MYBLASTWORLD";
		std::string templateName = "";

		size_t blastPos = cch.find("blast_id|");
		if (blastPos != std::string::npos) {
			size_t start = blastPos + 9;
			size_t end = cch.find('|', start);
			if (end != std::string::npos) {
				std::string blastStr = cch.substr(start, end - start);
				try {
					blastId = std::stoi(blastStr);
				}
				catch (const std::exception& e) {}
			}
		}

		size_t namePos = cch.find("world_name|");
		if (namePos != std::string::npos) {
			size_t start = namePos + 11;
			size_t end = cch.find('|', start);
			if (end != std::string::npos) {
				worldName = cch.substr(start, end - start);
			}
		...
		size_t templatePos = cch.find("template_name|");
		if (templatePos != std::string::npos) {
			size_t start = templatePos + 14;
			size_t end = cch.find('\n', start);
			if (end != std::string::npos) {
				templateName = cch.substr(start, end - start);
			}
			else {
				templateName = cch.substr(start);
			}
			if (!templateName.empty() && templateName.back() == '\r') {
				templateName.pop_back();
			}
		}

		if (cch.find("buttonClicked|edit_this_template") != std::string::npos) {
			showSingleBlastTemplateEditor(peer, blastId);
		//	return;
		}

		if (cch.find("buttonClicked|create_now") != std::string::npos) {
			World newWorld;

			if (!templateName.empty() && templateWorldExists(templateName)) {
				newWorld = load_template_world(templateName);
				newWorld.name = worldName;
				applyBlastModifications(peer, newWorld, blastId);
			}
			else {
				newWorld = create_world_blast(peer, worldName, blastId);
			}
			...
			gamepacket_t p;
			p.Insert("OnTextOverlay");
			p.Insert("`2Success! Created '" + worldName + "' with blast " + std::to_string(blastId));
			p.CreatePacket(peer);
		}
		else if (cch.find("buttonClicked|go_back") != std::string::npos) {
			std::string cmd = "/customblast";
			SendCmd(peer, cmd);
		}
	}

	if (cch.find("dialog_name|customblast_template_editor") != std::string::npos) {
		int selectedBlast = 0;
		std::vector<std::pair<std::string, int>> editButtons = {
			{"edit_9164", 9164}, {"edit_1402", 1402}, {"edit_1060", 1060},
			{"edit_8738", 8738}, {"edit_1532", 1532}, {"edit_830", 830},
			{"edit_10380", 10380}, {"edit_9602", 9602}, {"edit_1136", 1136},
			{"edit_3562", 3562}, {"edit_9582", 9582}, {"edit_7588", 7588},
			{"edit_6666", 6666}, {"edit_942", 942}, {"edit_4774", 4774},
			{"edit_7380", 7380}, {"edit_8556", 8556}
		};

		for (const auto& button : editButtons) {
			if (cch.find("buttonClicked|" + button.first) != std::string::npos) {
				selectedBlast = button.second;
				break;
			}
		...
		if (selectedBlast > 0) {
			showSingleBlastTemplateEditor(peer, selectedBlast);
		//	return;
		}
		else if (cch.find("buttonClicked|back_main") != std::string::npos) {
			std::string cmd = "/customblast";
			SendCmd(peer, cmd);
		//	return;
		}
	...
	if (cch.find("dialog_name|customblast_single_template") != std::string::npos) {
		int blastId = 9164;
		size_t blastPos = cch.find("embed_data|blast_id|");
		if (blastPos != std::string::npos) {
			std::string blastStr = cch.substr(blastPos + 20);
			blastStr = blastStr.substr(0, blastStr.find('|'));
			if (!blastStr.empty()) {
				try {
					blastId = std::stoi(blastStr);
				}
		...
		if (cch.find("buttonClicked|save_template") != std::string::npos) {
			std::string templateName = "";
			size_t pos = cch.find("new_template|");
			if (pos != std::string::npos) {
				size_t start = pos + 13;
				size_t end = cch.find('\n', start);
				if (end != std::string::npos) {
			...
			if (!templateName.empty()) {
				...
				templateName.erase(templateName.find_last_not_of(" \t\n\r\f\v") + 1);

				// Validasi: cek apakah template world ada di folder templates/
				if (!templateWorldExists(templateName)) {
					gamepacket_t p;
					p.Insert("OnTextOverlay");
					p.Insert("`4Template world '" + templateName + "' not found!");
					p.Insert("`7Make sure the file exists: templates/" + templateName + "_.json");
					p.CreatePacket(peer);
				}
				else {
					// Simpan mapping blast ID ke template name
					saveBlastTemplate(blastId, templateName);

					gamepacket_t p;
					p.Insert("OnTextOverlay");
					p.Insert("`2Template saved for " + getBlastDisplayName(blastId) + "!");
					p.Insert("`7Now using: " + templateName);
					p.CreatePacket(peer);
				}
				...
				gamepacket_t p;
				p.Insert("OnTextOverlay");
				p.Insert("`4Template name cannot be empty!");
				p.CreatePacket(peer);
			}
		}
		else if (cch.find("buttonClicked|clear_template") != std::string::npos) {
			clearBlastTemplate(blastId);

			gamepacket_t p;
			p.Insert("OnTextOverlay");
			p.Insert("`2Template cleared for " + getBlastDisplayName(blastId) + "!");
			p.Insert("`7Now using default generation");
			p.CreatePacket(peer);

			showBlastTemplateEditor(peer);
		}
		else if (cch.find("buttonClicked|back_editor") != std::string::npos) {
			showBlastTemplateEditor(peer);
		}
	}

	if (cch.find("dialog_name|editworldtemplate_main") != std::string::npos) {
		if (!pInfo(peer)->role.mod && !pInfo(peer)->role.dev) {
		//	return;
		}

		std::vector<std::string> templates = getTemplateWorlds();

		for (const auto& templateName : templates) {
			if (cch.find("buttonClicked|edit_" + templateName) != std::string::npos) {
				World templateWorld = load_template_world(templateName);

				if (templateWorld.blocks.empty()) {
					gamepacket_t p;
					p.Insert("OnConsoleMessage");
					p.Insert("`4Failed to load template world: " + templateName);
					p.CreatePacket(peer);
			//		return;
				}

				auto it = std::find_if(worlds.begin(), worlds.end(), [templateName](const World& w) { return w.name == templateName; });
				if (it != worlds.end()) {
					*it = templateWorld;
				}
				else {
					worlds.push_back(templateWorld);
				}

				std::string worldName = templateName;
				join_world_template(peer, worldName);

				gamepacket_t p;
				p.Insert("OnConsoleMessage");
				p.Insert("`2You have been sent to template world: " + templateName);
				p.Insert("`5Edit the world and use /savetemplate to save your changes.");
				p.CreatePacket(peer);
				//return;
			}
		}

		if (cch.find("buttonClicked|create_new") != std::string::npos) {
			showCreateTemplateDialog(peer);
			//return;
		}

		if (cch.find("buttonClicked|open_delete") != std::string::npos) {
			showDeleteTemplateDialog(peer);
		//	return;
		}

		if (cch.find("buttonClicked|refresh_list") != std::string::npos) {
			std::string cmd = "/editworldtemplate";
			SendCmd(peer, cmd);
			//return;
		}

		if (cch.find("buttonClicked|back_main") != std::string::npos) {
		//	return;
		}
	}

	if (cch.find("dialog_name|create_template_dialog") != std::string::npos) {
		if (!pInfo(peer)->role.mod && !pInfo(peer)->role.dev) {
			//return;
		}

		if (cch.find("buttonClicked|create_template") != std::string::npos) {
			std::string templateName = "";
			size_t pos = cch.find("new_template_name|");
			if (pos != std::string::npos) {
				size_t start = pos + 18;
				size_t end = cch.find('\n', start);
				if (end != std::string::npos) {
			...
			if (templateName.empty()) {
				std::string errorMsg = "msg|`4Error: Template name cannot be empty!`|\n";
				ENetPacket* packet = enet_packet_create(errorMsg.c_str(), errorMsg.length() + 1, ENET_PACKET_FLAG_RELIABLE);
			...
			if (templateName.find("..") != std::string::npos ||
				...
				templateName.find("\\") != std::string::npos) {
				std::string errorMsg = "msg|`4Error: Invalid template name!`|\n";
				ENetPacket* packet = enet_packet_create(errorMsg.c_str(), errorMsg.length() + 1, ENET_PACKET_FLAG_RELIABLE);
				enet_peer_send(peer, 0, packet);
			//	return;
			}
			createNewWorldTemplate(peer, templateName);
		}
		if (cch.find("buttonClicked|cancel_create") != std::string::npos) {
			showImprovedTemplateEditor(peer);
			//return;
		}
	}

	if (cch.find("dialog_name|delete_template_main") != std::string::npos ||
		cch.find("dialog_name|delete_template_empty") != std::string::npos) {
		...
		for (const auto& templateName : templates) {
			if (cch.find("buttonClicked|select_delete_" + templateName) != std::string::npos) {
				showDeleteConfirmationDialog(peer, templateName);
			//	return;
			}
		}

		if (cch.find("buttonClicked|back_editor") != std::string::npos) {
			std::string cmd = "/editworldtemplate";
			SendCmd(peer, cmd);
		//	return;
		}
	...
	if (cch.find("dialog_name|confirm_delete_template") != std::string::npos) {
		if (!pInfo(peer)->role.mod && !pInfo(peer)->role.dev) {
		//	return;
		}
		...
		// CARI TEMPLATE NAME DARI BUTTON NAME
		// Cari button yang diawali dengan "confirm_delete_"
		size_t buttonPos = cch.find("buttonClicked|confirm_delete_");
		if (buttonPos != std::string::npos) {
			size_t start = buttonPos + 29; // "buttonClicked|confirm_delete_" length = 29
			size_t end = cch.find('\n', start);
			if (end != std::string::npos) {
				templateName = cch.substr(start, end - start);
			}
		...
		std::cout << "=== DELETE DIALOG DEBUG ===" << std::endl;
		std::cout << "Full CCH: " << cch << std::endl;
		std::cout << "TemplateName found: '" << templateName << "'" << std::endl;

		if (!templateName.empty()) {
			std::string filePath = "templates/" + templateName + "_.json";
			std::cout << "Attempting to delete: " << filePath << std::endl;

			if (std::filesystem::exists(filePath)) {
				bool success = std::filesystem::remove(filePath);

				if (success) {
					// Hapus dari vector worlds
					auto it = std::find_if(worlds.begin(), worlds.end(),
						[templateName](const World& w) { return w.name == templateName; });
					if (it != worlds.end()) {
						worlds.erase(it);
					}
					...
					gamepacket_t p;
					p.Insert("OnConsoleMessage");
					p.Insert("`4Template '" + templateName + "' deleted successfully!");
					p.CreatePacket(peer);
					std::cout << "SUCCESS: Template deleted: " << templateName << std::endl;
				}
				else {
					gamepacket_t p;
					p.Insert("OnConsoleMessage");
					p.Insert("`4Failed to delete template '" + templateName + "'");
					p.CreatePacket(peer);
					std::cout << "ERROR: Failed to delete file: " << templateName << std::endl;
				}
			}
			else {
				gamepacket_t p;
				p.Insert("OnConsoleMessage");
				p.Insert("`4Template file not found: " + templateName);
				p.CreatePacket(peer);
		...
		//	return;
			...
			gamepacket_t p;
			p.Insert("OnConsoleMessage");
			p.Insert("`4Template name is empty!");
			p.CreatePacket(peer);
			std::cout << "ERROR: Template name is empty!" << std::endl;
			showDeleteTemplateDialog(peer);
			//return;
		}

		if (cch.find("buttonClicked|cancel_delete") != std::string::npos) {
			showDeleteTemplateDialog(peer);
		//	return;
		}
	}
	if (cch.find("action|dialog_return\ndialog_name|set_far") != std::string::npos) {
						...
						if (cch.find("buttonClicked|Claim_Your_Daily")) {
						...
						if (cch.find("action|dialog_return\ndialog_name|set_newbie_choice") != std::string::npos) {
							...
							if (packet::contains(cch, "buttonClicked|checkprice")) {
								...
								builder.add_label_icon(false, 20008, std::format("`${} `o(`w1`ox `w{} `oGems)", items[20008].name, setGems(global::check::price(20008))))
									.add_label_icon(false, 20032, std::format("`${} `o(`w1`ox `w{} `oGems)", items[20032].name, setGems(global::check::price(20032))))
									.add_label_icon(false, 20002, std::format("`${} `o(`w1`ox `w{} `oGems)", items[20002].name, setGems(global::check::price(20002))))
									.add_label_icon(false, 20036, std::format("`${} `o(`w1`ox `w{} `oGems)", items[20036].name, setGems(global::check::price(20036))))
									.add_label_icon(false, 20004, std::format("`${} `o(`w1`ox `w{} `oGems)", items[20004].name, setGems(global::check::price(20004))))
									.add_label_icon(false, 20006, std::format("`${} `o(`w1`ox `w{} `oGems)", items[20006].name, setGems(global::check::price(20006))));
							...
							if (packet::contains(cch, "chopper|") && parser.get_value("chopper") == "1") {
								if (inventory::contains(peer, 20008) == true) {
									pInfo(peer)->gems += inventory::count(peer, 20008) * global::check::price(20008);

									int removeItems = 0 - inventory::count(peer, 20008);

									modify_inventory(peer, 20008, removeItems);
									test++;
								}
							}
							if (packet::contains(cch, "sapphire|") && parser.get_value("sapphire") == "1") {
								if (inventory::contains(peer, 20032) == true) {
									pInfo(peer)->gems += inventory::count(peer, 20032) * global::check::price(20032);

									int removeItems = 0 - inventory::count(peer, 20032);

									modify_inventory(peer, 20032, removeItems);
									test++;
								}
							}
							if (packet::contains(cch, "golden|") && parser.get_value("golden") == "1") {
								if (inventory::contains(peer, 20002) == true) {
									pInfo(peer)->gems += inventory::count(peer, 20002) * global::check::price(20002);
									int removeItems = 0 - inventory::count(peer, 20002);

									modify_inventory(peer, 20002, removeItems);
									test++;
								}
							}
							if (packet::contains(cch, "emerald|") && parser.get_value("emerald") == "1") {
								if (inventory::contains(peer, 20036) == true) {
									pInfo(peer)->gems += inventory::count(peer, 20036) * global::check::price(20036);

									int removeItems = 0 - inventory::count(peer, 20036);

									modify_inventory(peer, 20036, removeItems);
									test++;
								}
							}
							if (packet::contains(cch, "ruby|") && parser.get_value("ruby") == "1") {
								if (inventory::contains(peer, 20004) == true) {
									pInfo(peer)->gems += inventory::count(peer, 20004) * global::check::price(20004);

									int removeItems = 0 - inventory::count(peer, 20004);

									modify_inventory(peer, 20004, removeItems);
									test++;
								}
							}
							if (packet::contains(cch, "diamond|") && parser.get_value("diamond") == "1") {
								if (inventory::contains(peer, 20006) == true) {
									pInfo(peer)->gems += inventory::count(peer, 20006) * global::check::price(20006);

									int removeItems = 0 - inventory::count(peer, 20006);

									modify_inventory(peer, 20006, removeItems);
									test++;
								}
							}
							set_bux(peer, pInfo(peer)->gems);
							...
							if (packet::contains(cch, "buttonClicked|instruction")) {
								...
								dialog.append("\nadd_label_with_icon|small|`$Chooper Ore Blocks: `oDrops 1-5 Chopper Ore. This ore's doesn't need any requirements.|left|20220|");
								dialog.append("\nadd_label_with_icon|small|`$Sapphire Ore Blocks: `oDrops 1-3 Sapphire Ore. This ore's doesn't need any requirements.|left|20222|");
								dialog.append("\nadd_label_with_icon|small|`$Golden Ore Blocks: `oDrops 1-2 Golden Ore. This ore's requires atleast Mining Pickaxe Level 2.|left|20224|");
								dialog.append("\nadd_label_with_icon|small|`$Emerald Ore Blocks: `oDrops 1 Emerald Ore. This ore's requires atleast Mining Pickaxe Level 3.|left|20226|");
								dialog.append("\nadd_label_with_icon|small|`$Diamond Ore Blocks: `oDrops 1 Diamond Ore. This ore's requires atleast Mining Pickaxe Level 5.|left|20228|");
								dialog.append("\nadd_spacer|small|");
								dialog.append("\nadd_textbox|`5About Pickaxes:|");
								dialog.append("\nadd_label_with_icon|small|`$Mining Pickaxes - Level 1: `oCan breaks Chopper Ore or Sapphire Ore blocks. This pickaxe can breaks up to 8 hits!|left|20014|");
								dialog.append("\nadd_label_with_icon|small|`$Mining Pickaxes - Level 2: `oCan breaks Golden Ore. This pickaxe can breaks up to 8 hits!|left|20022|");
								dialog.append("\nadd_label_with_icon|small|`$Mining Pickaxes - Level 3: `oCan breaks Emerald Ore. This pickaxe can breaks up to 7 hits!|left|20010|");
								dialog.append("\nadd_label_with_icon|small|`$Mining Pickaxes - Level 4: `oCan breaks fastly than other pickaxes. This pickaxe can breaks up to 5 hits!|left|20018|");
								dialog.append("\nadd_label_with_icon|small|`$Mining Pickaxes - Level 5: `oCan breaks Diamond Ore. This pickaxe can breaks up to 8 hits!|left|20020|");
								dialog.append("\nadd_spacer|small|");
						...
						/*else if (cch.find("action|dialog_return\ndialog_name|tesCalculators") != string::npos) {
						...
						else if (cch.find("action|dialog_return\ndialog_name|trash_itemx") != -1) {
							if (cch.find("buttonClicked|trash") != -1) {
								// Ambil semua item yang dipilih dari dialog
								std::vector<int> selected_items = extract_selected_items(cch);

								if (!selected_items.empty()) {
									for (int item_id : selected_items) {
										auto& inventory = pInfo(peer)->inv;

										// Mencari item berdasarkan ID dalam inventaris
										int idx_to_remove = -1;
										for (int i = 0; i < inventory.size(); ++i) {
											if (inventory[i].first == item_id) {
												idx_to_remove = i; // Temukan item yang cocok
												break;
											}
										}
										...
										if (idx_to_remove != -1) {
											...
											inventory.erase(inventory.begin() + idx_to_remove);
											variants::OnConsoleMessage(peer, "`2Item " + items[item_id].name + " berhasil dibuang.");
										}
										else {
											variants::OnConsoleMessage(peer, "`4Item " + items[item_id].name + " tidak ditemukan dalam inventaris.");
										}
									}
								}
								else {
									variants::OnConsoleMessage(peer, "`4Tidak ada item yang dipilih untuk dibuang.");
								}
							}
							}
						else if (cch.find("action|dialog_return\ndialog_name|daily_reward") != -1) {
							if (cch.find("buttonClicked|claim") != -1) {
								auto time_now = std::chrono::system_clock::now();
								auto time_wib = time_now + std::chrono::hours(7);
								std::time_t tt = std::chrono::system_clock::to_time_t(time_wib);
								tm local_tm = *std::localtime(&tt);
								int current_day = local_tm.tm_mday;

								if (pInfo(peer)->last_claimed_day == current_day) {
									variants::OnConsoleMessage(peer, "`4Kamu sudah mengambil daily reward hari ini!");
								}

								json daily_rewards;
								std::ifstream file("settings/daily_rewards.json");
								if (file.is_open()) {
									file >> daily_rewards;
									file.close();
								}
								else {
									variants::OnConsoleMessage(peer, "`4File daily_rewards.json tidak ditemukan!");
								}

								int item_id = 0;
								int item_count = 1;
								if (daily_rewards.contains(std::to_string(current_day))) {
									item_id = daily_rewards[std::to_string(current_day)]["item_id"];
									item_count = daily_rewards[std::to_string(current_day)]["item_count"];
								}

								if (item_id > 0 && item_count > 0 && pInfo(peer)->last_claimed_day != current_day) {
									modify_inventory(peer, item_id, item_count);
									variants::OnConsoleMessage(peer, "`2Kamu mendapatkan " + std::to_string(item_count) + "x " + items[item_id].name + " dari Daily Reward!");
									pInfo(peer)->last_claimed_day = current_day;
								}
								else {
									variants::OnConsoleMessage(peer, "`4Reward tidak ditemukan untuk hari ini!");
								}
							}
							}
									...
									if (verifiy_game = true) {
										if (!pInfo(peer)->tankIDName.empty() and pInfo(peer)->verifiedpl == false) {
											...
											dialog.add_custom_margin(10, 10)
												...
												.add_smallfont_button("loginozxkczokcozkcozkcx", "Next")
												.add_quick_exit()
												.add_spacer(false)
												.send(peer);
										}
									}
									if (pInfo(peer)->verifiedpl == true) {
										if_verified(peer);
									}
									if (pInfo(peer)->tankIDName.empty()) {
						...
						if (cch.find("buttonClicked|slotss") != string::npos) {
							SendCmd(peer, "/slots");
						}
						if (cch.find("buttonClicked|blackk_jakie") != string::npos) {
							SendCmd(peer, "/blackjack");
						}
						if (cch.find("buttonClicked|mines_game") != string::npos) {
							SendCmd(peer, "/mines");
						}
						if (cch.find("buttonClicked|diceee_rolller") != string::npos) {
							SendCmd(peer, "/diceroll");
						}
						if (cch.find("buttonClicked|fishing_game") != string::npos) {
							SendCmd(peer, "/fishgame");
						}
						if (cch.find("action|dialog_return\ndialog_name|set_block_drop") != std::string::npos) {
							bool isgacha = false;
							if (cch.find("gacha_i|1") != std::string::npos) {
								isgacha = true;
							}
							if (cch.find("gacha_i|0") != std::string::npos) {
								isgacha = false;
							}
							...
							// Tambahkan array kalau block belum ada
							...
							// Tambahkan drop baru dengan urutan field sesuai permintaan
							json newDrop = {
								{ "chance", chance },
								{ "count", count },
								{ "isgacha", isgacha },
							...
							jsonData[std::to_string(blockId)].push_back(newDrop);
						...
						if (cch.find("buttonClicked|") != std::string::npos) {
							string button = cch.substr(cch.find("buttonClicked|") + 13);

							if (button.rfind("song*", 0) == 0) { // Kalau mulai dengan "song*"
								int song_id = atoi(button.substr(5).c_str()); // Ambil angka setelah "song*"
								playsong(peer, song_id); // Panggil fungsi playsong yang kamu udah buat
							}
							else if (button == "settings") {
								gamepacket_t p;
								// Buka settings
								p.Insert("OnDialogRequest");
								p.Insert("set_default_color|`o\n\nadd_label_with_icon|big|`2Spotify Settings``|left|9492|\nadd_spacer|\nadd_textbox|`oWelcome to " + std::string(server_name) + " spotify you can listen to songs here, if you want to add your own song you will have to pay 1000 coins|\nadd_checkbox|shuffle|Shuffle Songs|" + to_string(pInfo(peer)->shuffle) + "\nadd_checkbox|loop_song|Loop selected song|" + to_string(pInfo(peer)->loopsong) + "\nadd_spacer|small|\nend_dialog|asspotifysettings|Close|Save|\n");
								p.CreatePacket(peer);
							}
							else if (button == "stopsong") {
								// Hentikan lagu
								stopsong(peer);
							}
						}
						...
						if (cch.find("action|dialog_return\ndialog_name|submit_items") != string::npos) {
							...
							config::one::set(itemid, farrx);
							config::two::set(itemid, gemss);
							config::three::set(itemid, hitxc);
							config::four::set(itemid, effect);
							config::five::set(itemid, effect2);
						...
						//else if (cch.find("action|dialog_return\ndialog_name|create_exchange") != std::string::npos) {
						//	std::string id = explode("\n", explode("id|", cch)[1])[0];
						//	std::string from_icon_str = explode("\n", explode("from_icon|", cch)[1])[0];
						//	std::string from_name = explode("\n", explode("from_name|", cch)[1])[0];
						//	std::string to_icon_str = explode("\n", explode("to_icon|", cch)[1])[0];
						//	std::string to_name = explode("\n", explode("to_name|", cch)[1])[0];
						//	std::string to_amount_str = explode("\n", explode("to_amount|", cch)[1])[0];

						//	// 🔒 Validasi input
						//	if (id.empty() || from_icon_str.empty() || from_name.empty() ||
						//		to_icon_str.empty() || to_name.empty() || to_amount_str.empty()) {
						//		variants::OnConsoleMessage(peer, "`4Error:`w All fields must be filled!");
						//		return;
						//	}

						//	// 🔢 Pastikan semua angka valid
						//	int from_icon, to_icon, to_amount;
						//	try {
						//		from_icon = std::stoi(from_icon_str);
						//		to_icon = std::stoi(to_icon_str);
						//		to_amount = std::stoi(to_amount_str);
						//	}
						//	catch (...) {
						//		variants::OnConsoleMessage(peer, "`4Error:`w Invalid number input!");
						//		return;
						//	}
						...
						//		if (ex["id"] == id) {
						//			variants::OnConsoleMessage(peer, "`4Error:`w Exchange with this ID already exists!");
						//			return;
						//		}
						//	}
						...
						//	// 🗂️ Pastikan folder settings ada
						//	std::filesystem::create_directories("settings");

						//	// 💾 Simpan file JSON
						//	std::ofstream outFile("settings/exchange.json");
						//	if (!outFile.is_open()) {
						//		variants::OnConsoleMessage(peer, "`4Error:`w Failed to save exchange file!");
						//		return;
						//	}
						//	outFile << exchanges.dump(4);
						//	outFile.close();
						...
						//	std::string msg = "`$>> Successfully added new exchange: `w" + from_name + "`$ → `w" + to_name + "`$ (x" + std::to_string(to_amount) + ")";
						//	variants::OnConsoleMessage(peer, msg);
						...
						//	p.CreatePacket(peer);
						//}
						//// Handler utama saat tombol exchange ditekan
						//else if (cch.find("buttonClicked|okijhuygtfrd") != std::string::npos) {
						//	DialogBuilder p;
						//	p.add_label_icon(true, 2250, "Welcome to Crystal Exchange of " + server_name + " Server")
						//		.add_smalltext("Here you can exchange your Crystals for Diamond Locks or other rewards!")
						//		.add_spacer(false)
						//		.add_custom_break()
						//		.add_text_input(3, "count_i", "Count of items you want to sell", "");

						//	// Baca semua item dari JSON
						//	nlohmann::json exchanges;
						//	std::ifstream file("settings/exchange.json");
						//	if (!file.is_open()) {
						//		p.add_smalltext("`4Error:`w No exchange data found. Use /addexchange first!");
						//		p.add_quick_exit().send(peer);
						//		return;
						//	}
						...
						//	}
						//	catch (...) {
						//		p.add_smalltext("`4Error:`w exchange.json file is corrupted!");
						//		file.close();
						//		p.add_quick_exit().send(peer);
						//		return;
						//	}
						...
						//	if (!exchanges.is_array() || exchanges.empty()) {
						//		p.add_smalltext("No available exchange items. Use /addexchange to create one!");
						//		p.add_quick_exit().send(peer);
						//		return;
						//	}

						//	// Loop semua data di JSON
						//	for (auto& item : exchanges) {
						//		if (!item.contains("id") || !item.contains("from_name") || !item.contains("to_name"))
						//			continue;
						...
						//		std::string btn_id = "buy_exc_" + id;
						//		std::string btn_label = "`$Exchange `w" + from_name + "`$ → `w" + to_name +
						//			" `o(" + std::to_string(from_amount) + " → " + std::to_string(to_amount) + ")";

						//		p.add_custom_break().add_button(btn_id, btn_label);
						//	}
						...
						//// Handler untuk semua tombol buy_exc
						//else if (cch.find("buttonClicked|buy_exc_") != std::string::npos) {
						//	std::string count_str = explode("\n", explode("count_i|", cch)[1])[0];
						//	int countx = std::stoi(count_str);
						//	if (countx <= 0) {
						//		console_msg(peer, "Invalid amount!");
						//		return;
						//	}

						//	std::string button_id = cch.substr(cch.find("buttonClicked|buy_exc_") + 20);
						//	std::string id = button_id.substr(0, button_id.find("\n")); // ambil ID exchange dari tombol

						//	nlohmann::json exchanges;
						//	std::ifstream file("settings/exchange.json");
						//	if (!file.is_open()) {
						//		console_msg(peer, "Exchange configuration not found!");
						//		return;
						//	}
						...
						//			int from_icon = item["from_icon"];
						//			int to_icon = item["to_icon"];
						//			int to_amount = item["to_amount"];
						//			std::string from_name = item["from_name"];
						//			std::string to_name = item["to_name"];

						//			int current_amount = inventory_contains(peer, from_icon);
						//			if (current_amount < countx) {
						//				console_msg(peer, "You don't have enough " + from_name + "!");
						//				return;
						//			}

						//			int total_to_give = to_amount * countx;
						//			int negativeCount = -countx;
						//			modify_inventory(peer, from_icon, negativeCount);
						//			modify_inventory(peer, to_icon, total_to_give);

						//			console_msg(peer, "You have successfully exchanged " + std::to_string(countx) + " " + from_name + " for " + std::to_string(total_to_give) + " " + to_name + "!");
						//			break;
						//		}
						//	}

						//	if (!found) {
						//		console_msg(peer, "Exchange type not found!");
						//	}
						//}
						...
						if (cch.find("buttonClicked|checkitemsdone") != string::npos) {
							SendCmd(peer, "/infoitems");
						}
						if (cch.find("buttonClicked|baclktooedititem") != string::npos) {
							SendCmd(peer, "/edititem");
						}
				...
				case 3: // world/enter
				{
					...
					if (cch == "action|quit") enet_peer_disconnect_later(peer, 0);
						...
						for (ENetPeer* currentr = server->peers; currentr < &server->peers[server->peerCount]; currentr++) {
							if (currentr->state != ENET_PEER_STATE_CONNECTED || pInfo(currentr)->carniwaitgame == "" || pInfo(currentr)->world != pInfo(peer)->world) continue;
							if (pInfo(currentr)->carniwait != 0 && pInfo(currentr)->carniwaitgame == pInfo(peer)->carnival_playing) pInfo(currentr)->carniwait--;
							if (pInfo(currentr)->carniwait == 0 && pInfo(currentr)->carniwaitgame == pInfo(peer)->carnival_playing)	carnivalStart(currentr, pInfo(currentr)->carniwaitgame + "_start");
						}
						if (!packet::contains(pInfo(peer)->carnival_playing, "Growganoth_")) {
							pInfo(peer)->carnivalscore = 0;
							pInfo(peer)->carnival_playing = "";
							pInfo(peer)->carniwait = 0;
							pInfo(peer)->carniwaitgame = "";
							carnivalRespawn(peer);
						}
						exit_(peer);
					}
				...
				case 4:
				{
					...
					case 0: /*Kai zaidejas pajuda*/
					{
						...
						if (pInfo(peer)->world == "CARNIVAL" && Carnival) {
							string name_ = pInfo(peer)->world;
							vector<World>::iterator p = find_if(worlds.begin(), worlds.end(), [name_](const World& a) { return a.name == name_; });
							if (p != worlds.end()) {
								World* world_ = &worlds[p - worlds.begin()];
								...
								int y_ = (int)p_->y / 32;
								WorldBlock* block_ = &world_->blocks[x_ + (y_ * 100)];
								if (block_->fg == 1908 && pInfo(peer)->carnival_playing == "Mirror_Maze") {
									if ((world_->blocks[(int)x_ + ((int)y_ * 100)].flags & 0x00400000) != 0) {
										carnivalPrize(peer);
										carnivalEnd(peer, "end");
										block_->flags = block_->flags ^ 0x00400000;
										PlayerMoving data_{};
										data_.packetType = 5, data_.punchX = x_, data_.punchY = y_, data_.characterState = 0x8;
										BYTE* raw = packPlayerMoving(&data_, 112 + alloc_(world_, block_)); BYTE* blc = raw + 56;
										...
										for (size_t i = 0; i < pInfo(peer)->mirrorMaze.Carnival_Mirror_Maze.size(); ++i) {
											int x = pInfo(peer)->mirrorMaze.Carnival_Mirror_Maze[i].first;
											int y = pInfo(peer)->mirrorMaze.Carnival_Mirror_Maze[i].second;
											string name_ = "CARNIVAL";
											vector<World>::iterator p = find_if(worlds.begin(), worlds.end(), [name_](const World& a) { return a.name == name_; });
											if (p != worlds.end()) {
												World* world_ = &worlds[p - worlds.begin()];
												for (int i_ = 0; i_ < world_->blocks.size(); i_++) {
													WorldBlock* block_ = &world_->blocks.at(x + (y * 100));
													if (block_->fg == 1926 and (world_->blocks[(int)x + ((int)y * 100)].flags & 0x00400000) != 0) {
														block_->flags = block_->flags ^ 0x00400000;
														PlayerMoving data_{};
														data_.packetType = 5, data_.punchX = x, data_.punchY = y, data_.characterState = 0x8;
														BYTE* raw = packPlayerMoving(&data_, 112 + alloc_(world_, block_)); BYTE* blc = raw + 56;
								...
								if (pInfo(peer)->carnival_playing == "Brutal_Bounce") {
									if ((rand() % 200) <= 10 and world_->blocks[(int)x_ + ((int)y_ * 100)].fg == 0) {
										block_->fg = 2568;
										PlayerMoving data_{};
										data_.packetType = 5, data_.punchX = x_, data_.punchY = y_, data_.characterState = 0x8;
										BYTE* raw = packPlayerMoving(&data_, 112 + alloc_(world_, block_)); BYTE* blc = raw + 56;
								...
								if (pInfo(peer)->carnival_playing == "Mirror_Maze" && !pInfo(peer)->mirrorMaze.Carnival_Mirror_Reset) {
									if (pInfo(peer)->mirrorMaze.Carnival_Mirror_Maze.size() < 1) break;
									pInfo(peer)->mirrorMaze.Carnival_Mirror_Reset = true;
									{
										int x = 24,
											y = 24;
										WorldBlock* block_ = &world_->blocks.at(x + (y * 100));
					...
					case 7: /*Kai zaidejas ieina pro duris arba portal*/ /*2/16/2022 update: cia dar gali buti STEAM USE*/
					{
						...
						if (p != worlds.end()) {
							...
							else if (items[block_->fg].blockType == BlockTypes::GAME_BLOCK) {
								if (world_->game.started) {
									auto player = world_->game.GetPlayer(pInfo(peer)->netID);
									if (player == nullptr) break;
									if (block_->fg == 1622) {
										if (block_->id != player->team and player->capture.second == -1) {
											int blockIndex = x + (y * 100);
											world_->game.manageBattle(peer, CAPTURE, pInfo(peer)->netID, blockIndex);
											player->capture = std::make_pair(std::make_pair(x, y), block_->id);
											world_->game.destroyed_blocks.push_back(
												{
													.x = (int8_t)x,
													.y = (int8_t)y,
													.teamId = (int8_t)block_->id,
													.foreground = block_->fg,
													.background = block_->bg,
													.flags = (uint32_t)block_->flags
												}
											);
										}
									}
									else if (block_->fg == 1658) {
										if (world_->game.pv_goal != 0) {
											world_->game.ModifyPlayer(peer, SCORE, world_->game.pv_goal);
											...
											packet.m_type = 17;
											packet.m_particle_emitter_id = 69;
											packet.m_vec_x = x * 32 + 16;
											packet.m_vec_y = y * 32 + 16;
											packet.m_particle_variable = player->team;
											packet.m_particle_alt_id = 69;
											for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
												if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL) continue;
												if (pInfo(currentPeer)->world == pInfo(peer)->world) {
										...
										if (player->capture.second != -1) {
											WorldBlock* block2 = &world_->blocks[player->capture.first.first + (player->capture.first.second * 100)];
											PlayerMoving data_{};
											data_.packetType = 5, data_.punchX = static_cast<int>(player->capture.first.first), data_.punchY = static_cast<int>(player->capture.first.second), data_.characterState = 0x8;
											BYTE* raw = packPlayerMoving(&data_, 112 + alloc_(world_, block2));
											BYTE* blc = raw + 56;
											form_visual(blc, *block2, *world_, peer, false);
											for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
												if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL) continue;
												if (pInfo(currentPeer)->world == pInfo(peer)->world) {
													send_raw(currentPeer, 4, raw, 112 + alloc_(world_, block2), ENET_PACKET_FLAG_RELIABLE);
												}
											}
											delete[] raw, blc;
											if (block_->locked) {
												upd_lock(*block2, *world_, peer);
											}

											if (world_->game.pv_flag != 0) {
												world_->game.ModifyPlayer(peer, SCORE, world_->game.pv_flag);
												if (world_->game.isOptionEnabled(RESPAWN_ON_SCORE))
													SendRespawn(peer, false, 1, 0);
											...
											world_->game.manageBattle(peer, UNK_VAL, pInfo(peer)->netID);
											player->capture = std::make_pair(std::make_pair(-1, -1), -1);
											break;
										}
									}
								}
							...
							/*
								...
								if (world_->name == "CARNIVAL" && Carnival) {
									bool notick = false;
									int gtick = 0;
									struct tm newtime;
									time_t now = time(0);
									...
									time(&currentTime);
									const auto localTime = localtime(&currentTime);
									modify_inventory(peer, 1898, gtick);
									if (block_->fg == 1910 and gtick == 0) notick = true;
									if (notick && (Carnival || localTime->tm_mday >= 27)) {
										send_bubble(peer, pInfo(peer)->netID, "`4You dont have any Golden Ticket.");
									}
									if (not Carnival and localTime->tm_mday <= 27 and block_->fg == 1910) notick = true, send_bubble(peer, pInfo(peer)->netID, "Carnival is not opened yet!");
									if (block_->door_destination.find(":GAME") != std::string::npos) {
										int count = -1;
										if (modify_inventory(peer, 1898, count) == 0) {
											door_join_carnival(peer, world_, block_, block_->door_destination, notick);
										}
										else {
											gamepacket_t p2;
									...
									if (locked == false) {
										if (not door_id.empty()) {
											...
											if (p != target_world.blocks.end()) {
												int i_ = p - target_world.blocks.begin();
												spawn_x = i_ % 100, spawn_y = i_ / 100;
												if (pInfo(peer)->carnival_playing == "Mirror_Maze") spawn_x = 14, spawn_y = 32;
												if (pInfo(peer)->carnival_playing == "Spiky_Survival") spawn_x = 21, spawn_y = 41;
												if (pInfo(peer)->carnival_playing == "Brutal_Bounce") spawn_x = 81, spawn_y = 25;
												if (pInfo(peer)->carnival_playing == "Card_Left") spawn_x = 40, spawn_y = 31;
												if (pInfo(peer)->carnival_playing == "Card_Right") spawn_x = 64, spawn_y = 31;
												if (pInfo(peer)->carnival_playing == "Shooter_Left") spawn_x = 38, spawn_y = 53;
												if (pInfo(peer)->carnival_playing == "Shooter_Right") spawn_x = 70, spawn_y = 53;
								...
								bool block_a = false;
								if (((block_->fg == 10358 or block_->fg == 21356) && block_access(peer, world_, block_) == false) && block_->open && block_->shelf_1 != 0) {
									/*int my_wls = get_wls(peer, true);
									if (block_->fg == 21356) my_wls = pInfo(peer)->gems;
									if (my_wls >= block_->shelf_1) {
										if (block_->fg == 10358) get_wls(peer, true, true, block_->shelf_1);
										if (block_->fg == 21356) pInfo(peer)->gems -= block_->shelf_1;
									...
									if (entrybooth_player(peer, block_) == true) {
										block_a = false;
									}
									else {
										block_a = true;
					...
					case 23: {
						GUP::GameUpdatePacket* packet = GUP::DataToUpdatePacket(event.packet);
						...
						if (p != worlds.end()) {
							...
							if (world_->game.started) {
								...
								for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
									...
									if (pInfo(currentPeer)->netID != packet->m_target_net_id) continue;
									auto attackerPlayer = world_->game.GetPlayer(packet->m_target_net_id);
									if (attackerPlayer == nullptr || gamePlayer == nullptr) continue;
									if (attackerPlayer->team == gamePlayer->team && attackerPlayer->team != 4) continue;
									gamePlayer->hits++;

									if (gamePlayer->hits >= world_->game.player_hits) {
										SendRespawn(peer, false, 0, 1);

										world_->game.ModifyPlayer(currentPeer, SCORE, world_->game.pv_kill);

										GUP::GameUpdatePacket packet;
										packet.m_type = 17;
										packet.m_particle_emitter_id = 69;
										packet.m_vec_x = pInfo(peer)->x;
										packet.m_vec_y = pInfo(peer)->y;
										packet.m_particle_variable = attackerPlayer->team;
										packet.m_particle_alt_id = 69;
										for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
											if (currentPeer->state != ENET_PEER_STATE_CONNECTED or currentPeer->data == NULL) continue;
											if (pInfo(currentPeer)->world == pInfo(peer)->world) {
										...
										world_->game.ModifyPlayer(peer, HIT, -gamePlayer->hits);

										if (world_->game.lives > 0) {
											world_->game.ModifyPlayer(peer, LIVE, -1);
											if (gamePlayer->lives <= 0) {
												world_->game.manageBattle(peer, LEAVE, gamePlayer->netID);
											}
										}
									}
								}
