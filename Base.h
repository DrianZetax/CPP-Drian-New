...
namespace global::temp {
	std::vector<std::string> playerScanner, worldScanner;
}

ENetHost* server;
string documents = "C:/Users/Admin/Desktop/SERVER";
int i1j = 5194, i2j = 5176, i3j = 8430, i4j = 2204, i5j = 3584, i6j = 262;
int i1c = 200, i2c = 100, i3c = 1, i4c = 3, i5c = 200, i6c = 3;
int r1 = 12996, r1c = 1;
vector<int>random_shop_item;
vector<int> dstone;
bool Carnival = true; // EVENT
...
int get_card_icon(int val) {
	// Sesuaikan ID kartu dengan angka 1–9
	switch (val) {
	case 1: return 2816;
	case 2: return 2818;
	case 3: return 2820;
	...
	case 8: return 2830;
	case 9: return 2832;
	default: return 7246; // icon back card
	}
}

map<int, Song> musics{
	{1, {"El Raton - `5El Komamder", 200}},
	{2, {"Obsesion - `5Aventura", 256}},
	{3, {"I'll Be Missing You - `5Diddy, Faith Evans, 112", 236}},
	{4, {"A Thousand Miles - `5Vannesa Carlton", 267}},
	{5, {"Elevate - `5Nemzzz", 146}},
	{6, {"Smile - `5Lily Allen", 200}},
	{7, {"A Teen - `5V.I.Cz", 202}},
	{8, {"Starships - `5Nicki Minaj", 262}},
	{9, {"Pineapple Disco 2022 - `5Growtopia Originals", 148}},
	{10, {"Startopia Galactic Nexus Theme - `5Growtopia Originals", 58}},
};
struct DailyReward {
	int day;         // Hari ke-berapa (1-31)
	int itemID;      // ID item hadiah
	int count;       // Jumlah item
	int gemReward;   // Gems reward (optional, bisa 0)
};

std::vector<DailyReward> dailyRewards = {
	{1, 18, 5, 500},
	{2, 32, 3, 800},
	{3, 242, 1, 1500},
	// ...
...
struct OPCSHOP_ITEM {
	...
	int price_item; // Harga dalam OPC Token
};
extern std::vector<OPCSHOP_ITEM> g_opc_shop_items;
std::vector<OPCSHOP_ITEM> g_opc_shop_items;
...
vector<vector<string>> info_about_playmods{
	...
	{ "153", "10400", "604800", "Cheater Role Ĝ", "You have received cheater role!", "Cheater role is over...", "10400", "", "spray.wav", "", "drop", "", "", "" },
...
string parseText_2(string cch, string get, string delimiter = "\n") {
	size_t start = cch.find(get) + get.length(), end = cch.find(delimiter, start);
	if (end == string::npos) end = cch.length();
	return cch.substr(start, end - start);
}	


inline vector<string> explodes(const string& delimiter, const string& str) {
	vector<string> arr;
...
int items_dat() {
  ...
  if (def.breakHits <= 0 or def.breakHits > 42 * 6)
   def.breakHits = 42 * 6;
		...
		if (def.actionType == 127 || def.actionType == 126 || def.actionType == 118 || def.id == 718 || def.actionType == 117 || def.actionType == 116 || def.actionType == 115 || def.actionType == 113 || def.actionType == 109 || def.actionType == 106 || def.actionType == 105 || def.actionType == 104 || def.actionType == 102 || def.actionType == 99 || def.actionType == 96 || def.actionType == 86 || def.actionType == 80 || def.actionType == 79 || def.actionType == 78 || def.actionType == 75 || def.actionType == 73 || def.actionType == 72 || def.actionType == 71 || def.actionType == 68 || def.actionType == 66 || def.actionType == 65 || def.actionType == 57 || def.actionType == 53 || def.actionType == 52 || def.actionType == 50 || def.actionType == 43 || def.actionType == 40 || def.actionType == 91 || def.id == 5818 || def.id == 5820 || def.id == 6280) def.blocked_place = true;
		...
		if (i == 1796) items[i].max_gems3 = 1000;
		if (i == 7188) items[i].max_gems3 = 10000;
		...
		if (i == 9814) items[i].randomitem = { 4534, 4628, 5020, 5206, 5322, 5712, 5480, 6294, 6338, 6758, 6892, 7196, 7384, 7568, 7570, 7572, 7574, 7676, 7678, 7680, 7682, 7836, 7838, 7840, 7842, 8006, 8010, 8288, 8290, 8292, 8294, 8304, 8432, 8434, 8436, 8438, 8576, 8578, 8580, 8816, 8818, 8820, 8822, 7192, 8898, 8900, 8902, 8904, 9008, 9010, 9012, 9014, 9116, 9118, 9120, 9122, 9136, 9138, 9236, 9408, 9462, 9464, 9606, 9648, 9760, 10044, 10128, 10166, 10426, 10618, 10666, 10718, 10810, 11006, 11116, 11232, 11306, 11440, 11506, 11704, 11750, 11882, 11906, 12246, 12344, 12432, 12642, 12834, 12872, 13022, 13106, 13460, 10364, 13258, 2002, 5638, 3604, 3402, 4896, 4902, 7534, 10666, 13038, 6008, 11220, 7120, 8996, 6848, 10336, 12308, 5638 };
		// Misc
		...
		if (i == 12408 || i == 12902 || i == 13002 || i == 7152) items[i].oprc = 100;
	...
	delete[] data;
	ifstream file_info("equipment/playmods.txt");
	if (file_info.is_open()) {
		...
		file_info.close();
		{
			ifstream file_info("equipment/items/en");
			if (file_info.is_open()) {
				string line_info;
				while (getline(file_info, line_info)) {
			...
			file_info.close();
			{
				ifstream file_info("equipment/farmables.txt");
				if (file_info.is_open()) {
					string line_info;
					while (getline(file_info, line_info)) {
				...
				file_info.close();
				{
					ifstream file_info("equipment/combiner.txt");
					if (file_info.is_open()) {
						string line_info, a = "";
						while (getline(file_info, line_info)) {
	...
	for (vector<vector<int>>recipe : receptai) {
		...
		items[recipe[3][0]].disableFind = true;
	}
	for (vector<vector<int>>crystal : crystal_receptai) {
		...
		items[crystal[5][0]].disableFind = true;
	}
	...
	for (int i = 0; i < itemCount; i++) {
		...
		if (packet::contains(items[i].name, "wings") || item_playmod(i, "double jump")) items[i].playmod_total += "double jump";
		...
		if (packet::contains(items[i].name, "Mining Pickaxe")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Crate")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Easter")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Egg")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Guild")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Golden")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Mythical")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Phoenix")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Legendary")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Rayman")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Neptune")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Dumb")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Rod")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Anomalizing")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Pristine")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Elegant")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Exquisite")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Crystal")) items[i].disableFind = true;
		if (packet::contains(items[i].name, " Ore")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Inviso")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Bedrock")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Shard")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Gems")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "The Miners")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Main Door")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Camera Log")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Subscription") || packet::contains(items[i].name, "Wand")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Voucher") || packet::contains(items[i].name, "Coupon") || packet::contains(items[i].name, "Gem")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Snack") || packet::contains(items[i].name, "Stuff")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Ultraviolet") || packet::contains(items[i].name, "Ultra Zeus")) items[i].disableFind = true;
		if (packet::contains(items[i].name, "Diamond Horn") || packet::contains(items[i].name, "Diamond Devil Horn")) items[i].disableFind = true;
		if (items[i].blocked_place || items[i].toggleable || items[i].blockType == LOCK) items[i].disableFind = true;
		// End of Custom Items;

		if (i == 13802 || i == 10126 || i == 9864 || i == 244 || i == 3686 || i == 8 || i == 8470) items[i].disableFind = true;
		if (i == 8440 || i == 10362 || i == 8358 || i == 10124 || i == 8994) items[i].disableFind = true;
		if (i == 9840 || i == 10366 || i == 11234 || i == 11236 || i == 10930) items[i].disableFind = true;
		if (i == 4590 || i == 11318 || i == 13686 || i == 9586 || i == 396 || i == 13606) items[i].disableFind = true;
		if (i == 9860 || i == 9912 || i == 9874 || i == 10388 || i == 9594 || i == 13638) items[i].disableFind = true;
		if (i == 9552 || i == 5172 || i == 5190 || i == 836 || i == 13700 || i == 10122) items[i].disableFind = true;
		if (i == 10118 || i == 10252 || i == 5826 || i == 8372 || i == 6840 || i == 10364) items[i].disableFind = true;
		if (i == 1396 || i == 1492 || i == 6 || i == 1354 || i == 1310 || i == 7416) items[i].disableFind = true;
		if (i == 10250 || i == 10332 || i == 9376 || i == 11076 || i == 9412) items[i].disableFind = true;
		//raid by vyte 

		if (i == 9270 || i == 392 || i == 3402 || i == 9350 || i == 9976 || i == 9572 || i == 720) items[i].disableFind = true;
		if (i == 836 || i == 10294 || i == 1618 || i == 10716) items[i].disableFind = true;
		if (i == 8522 || i == 10836 || i == 10838 || i == 10840 || i == 10834 || i == 10842 || i == 12678) items[i].disableFind = true;
		if (i == 7628 || i == 5136 || i == 9386 || i == 9384 || i == 15018 || i == 9380 || i == 11156 || i == 9600 || i == 13160 || i == 9814) items[i].disableFind = true;
		if (i == 8426 || i == 13550 || i == 542 || i == 12260 || i == 12252 || i == 9566 || i == 9568 || i == 3604 || i == 10004 || i == 11038) items[i].disableFind = true;
		if (i == 9530 || i == 10826 || i == 718 || i == 9616 || i == 1584 || i == 7960 || i == 2478) items[i].disableFind = true;
		if (items[i].properties & Property_AutoPickup || items[i].blockType == SEED || items[i].blockType == GAME_BLOCK || items[i].blockType == GAME_GENERATOR || items[i].untradeable) items[i].disableFind = true;
		if (!items[i].noob_item.empty() || !items[i].rare_item.empty() || items[i].newdropchance != 0) items[i].disableFind = true;

		if (i == 13317 || i == 13319 || i == 1746 || i == 1830 || i == 1966 || i == 6840 || i == 1068 || i == 5090 || i == 3088 || i == 5198) items[i].disableFind = true;
		if (i == 9554 || i == 8440 || i == 9548 || i == 9812 || i == 9878 || i == 9874 || i == 9880 || i == 3932 || i == 3934 || i == 9568 || i == 9566) items[i].disableFind = true;
		if (i == 8440 || i == 9522 || i == 7782 || i == 7784 || i == 5138 || i == 5136 || i == 11550 || i == 14522) items[i].disableFind = true;
		...
		if (items[i].r_1 != 0 && items[i].r_2 != 0) {
		...
		if (items[i].unobtainable == false && items[i].untradeable == 0) {
			...
			if (i != 9852 and i != 8470) {
				if ((items[i].blockType == BlockTypes::LOCK or items[i].clothingType == 5 or items[i].clothingType == 6) and i % 2 == 0 and i != 5814 and i != 9852 and i != 9726 and i != 15000 and i != 15036 and i != 7782 and i != 7784 and i != 5138 and i != 5136 and i != 11550 and i != 14522) items[15018].randomitem.push_back(i);
			}
		}
		if (items[i].blockType == BlockTypes::CONSUMABLE and i != 6336 and i != 7480 and i != 9140 and i != 9110 and i != 9112 and i != 9852 and i != 7782 and i != 7784 and i != 5138 and i != 5136 and i != 11550 and i != 14522 and items[i].untradeable == 0)items[9600].randomitem.push_back(i);
		...
		if (i != 5816 && i != 1424 && items[i].untradeable != 1 && i != 9380) {
		...
		if (items[i].blockType == BlockTypes::SEED || items[i].rarity < 1 || items[i].rarity == 999 || items[i].block_possible_put == false || items[i].r_1 == 2037 || items[i].r_2 == 2037 || items[i].r_1 == 2035 || items[i].r_2 == 2035 || items[i].r_1 + items[i].r_2 == 0 || items[i].blockType == BlockTypes::CLOTHING || items[i].untradeable || i == 15036 || i == 15000 || i == 1424 || i == 5816 || i == 7782 || i == 7784 || i != 5138 || i != 5136 || i != 11550 || i != 14522 || items[i].rarity > 200) {
		}
		else {
			items_washing.push_back(make_pair(items[i].rarity, i));
			items[i].disableFind = true;
		}
	}
...
std::vector<int> extract_selected_items(const std::string& cch) {
	std::vector<int> selected_items;
	std::regex reg("\\b([0-9]+)\\b");
	std::smatch match;

	auto begin = cch.begin();
	auto end = cch.end();
	while (std::regex_search(begin, end, match, reg)) {
		selected_items.push_back(std::stoi(match[0]));
...
std::string getBlockSound(int const& itemId) {
	if (!packet::contains(items[itemId].extraFile, "audio/")) return "";

	return items[itemId].extraFile;
}
...
namespace cch {
	string substr(const string& cch, const string& target) {
		size_t pos = cch.find(target);
		if (pos == string::npos) {
			return "";
		}

		pos += target.length();
		string world = cch.substr(pos);
		world.erase(0, world.find_first_not_of(" \n\r\t"));
		cout << world << endl;
		return world;
	}
}
...
public:
	...
	std::vector<std::string> get_all_array() {
		...
		for (int i = 0; i < m_data.size(); i++) ret.push_back(format("[{}]: {}", i, m_data[i]));
		return ret;
	}
